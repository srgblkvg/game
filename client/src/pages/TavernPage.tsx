    ...
    return (
        ...
        <div className="max-w-3xl mx-auto px-4 py-4">
          ...
          {actionCard && <PageHeader title="Трактир" icon={actionCard.icon} bgImage={actionCard.bg_image} />}
            ...
            {tab==='heal' && <><p className="text-xs text-[var(--color-text-muted)] bg-[var(--color-bg-secondary)] rounded p-2 mb-3">Восстановите здоровье за монеты. Быстрое лечение — половина недостающего HP, полное — всё сразу.</p><Card><h3 className="font-bold mb-2">Мгновенное лечение</h3><p className="text-xs text-[var(--color-text-muted)] mb-3">Недостаёт HP: {missingHp} (2 монеты за HP)</p><div className="flex gap-3 flex-wrap"><Button variant="danger" fullWidth onClick={()=>handleHeal(false)} disabled={missingHp<=0}>50% — {formatMoney(Math.ceil(missingHp*0.5)*2)}</Button><Button variant="danger" fullWidth onClick={()=>handleHeal(true)} disabled={missingHp<=0}>Всё — {formatMoney(missingHp*2)}</Button></div>{(document.documentElement.classList.contains('vk-iframe')) && <div className="mt-3 pt-3 border-t border-[var(--color-border-light)]">{(() => { const presentation = getAdHealPresentation({ missingHp, loading: adLoading, cooldown: adCooldown }); return <><Button variant="primary" fullWidth onClick={handleAdHeal} disabled={presentation.disabled}>{presentation.buttonText}</Button><p className="text-[0.6rem] text-[var(--color-text-muted)] mt-1">{presentation.hintText}</p></> })()}</div>}</Card></>}
