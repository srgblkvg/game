...
export default function TavernPage() {
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 py-4">
            ...
            {tab==='drink' && <><p className="text-xs text-[var(--color-warning-text)] bg-[var(--color-accent-warning)]/10 border border-[var(--color-accent-warning)]/20 rounded p-2 mb-3">⚠️ Можно находиться под действием только одного напитка.</p>{(() => { const cats: Record<string, any[]> = {}; for (const d of tavern.drinks) { const c = d.category || 'Прочее'; if (!cats[c]) cats[c] = []; cats[c].push(d); } return Object.entries(cats).map(([cat, items]) => { const collapsed = collapsedCats[cat]; return <div key={cat} className="mb-4"><h3 className="text-sm font-bold mb-2 text-[var(--color-accent-info)] cursor-pointer select-none flex items-center gap-1" onClick={() => setCollapsedCats(prev => ({ ...prev, [cat]: !prev[cat] }))}><Icon icon={collapsed ? 'game-icons:plus' : 'game-icons:minus'} width="14" height="14" />{cat}</h3>{!collapsed && <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{items.map((drink:any)=><Card key={drink.key} className="flex flex-col"><div className="w-full h-20 bg-[var(--color-bg-input)] rounded mb-2 flex items-center justify-center text-2xl"><Icon icon="game-icons:potion-ball" width="32" height="32"/></div><h3 className="font-bold text-xs mb-1">{drink.name}</h3><div className="text-[0.65rem] text-[var(--color-text-muted)] mb-2 flex-1">{Object.entries(drink.bonuses as Record<string,number>).map(([k,v])=><span key={k} className="block">{statNames[k]||k}: +{v}</span>)}<span className="block">1 час</span></div><Button variant="danger" size="xs" fullWidth onClick={()=>handleDrink(drink.key)}>{formatMoney(drink.cost)}</Button></Card>)}</div>}</div>; }) })()}</>}
