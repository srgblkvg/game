...
export default function TavernPage() {
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 py-4">
            ...
            <div className="flex gap-2 mb-4">{(['heal','room','drink','quests'] as const).map(t => <Button key={t} variant={tab===t?'primary':'secondary'} size="sm" onClick={()=>setTab(t)}>{t==='heal'?'Лечение':t==='room'?'Комнаты':t==='drink'?'Напитки':'Задания'}</Button>)}</div>
                    ...
                    {quests.quests.map((q: any) => (
                        <Card key={q.id} className={q.status==='active'?'border-[var(--color-accent-info)]':''}>
                            ...
                            <div className="flex gap-2">
                                {q.status==='available' && quests.canTake && <Button variant="primary" size="sm" onClick={()=>handleTakeQuest(q.id)}>Взять</Button>}
                                {q.status==='active' && q.progress >= q.requirement && <Button variant="danger" size="sm" onClick={()=>handleClaimQuest(q.id)}>Сдать</Button>}
                            </div>
                        </Card>
                    ))}
                </div>
