...
import { fetchAuction, fetchAuctionHistory, fetchMyAuctionLots } from '../api/auction';
  ...
  useEffect(() => { fetch('/api/actions', { headers: getHeaders() }).then(r => r.json()).then((cards: any[]) => { const c = cards.find((x: any) => x.path === '/auction'); if (c) setActionCard(c); }).catch(() => {}); }, []);
    ...
    const load = async (pg?: number, groupKey?: string) => {
        ...
        try {
            ...
            const data = await fetchAuction(qs);
            setLots(data.lots);
    ...
    const loadHistory = async (p = 1) => {
        try { const data = await fetchAuctionHistory(p); setHistory(data.history); setHistoryPage(data.page || 1); setHistoryTotalPages(data.totalPages || 1); }
        catch { setHistory([]); }
    };
    ...
    const loadMyLots = async () => {
        ...
        try {
            const data = await fetchMyAuctionLots();
            const ownLots = data.lots;
            setMyLots(ownLots);
                    ...
                    {isMaterial && maxItemCount > 1 && (
                        <div className="mb-2">
                            ...
                            <div className="flex gap-2 items-center">
                                ...
                                <input type={isVk ? "text" : "number"} inputMode={isVk ? "none" : undefined} data-vk-num={isVk ? "true" : undefined} autoComplete="off" min={1} max={maxItemCount} value={sellCount} onChange={e => { const v = parseInt(e.target.value); if (!isNaN(v)) handleCountChange(Math.max(1, Math.min(maxItemCount, v))); }} className={inputClass + ' w-16 text-center'} />
                    ...
                    {loading ? (
                        ...
                        (() => {
                            ...
                            return (
                                ...
                                <Card className={`mb-3 ${String(lot.id) === highlightLotId ? '!border-2 !border-[#f59e0b] shadow-[0_0_10px_rgba(245,158,11,0.6)]' : ''}`}>
                                        ...
                                        <div className="flex flex-col items-end gap-2 shrink-0">
                                            ...
                                            {lot.sellerId !== character?.id && (
                                                <div className="flex flex-col gap-1.5 items-end">
                                                    {lot.buyoutPrice && (
                                                        <Button size="md" variant="primary" onClick={() => handleBuyout(lot.id)} disabled={(character?.money || 0) < lot.buyoutPrice}>
                                                            Выкупить {formatMoney(lot.buyoutPrice)}
                                                        </Button>
                                                    )}
                                                    <div className="flex items-end gap-1">
                                                        <input type={isVk ? "text" : "number"} inputMode={isVk ? "none" : undefined} data-vk-num={isVk ? "true" : undefined} autoComplete="off" placeholder={String(minBid)} value={bidAmount[lot.id] || ''}
                                                        ...
                                                        <Button size="md" variant="secondary" onClick={() => handleBid(lot.id, bidAmount[lot.id] || String(minBid), minBid)}>Ставка</Button>
                                                    </div>
                                                    {isStack && lot.buyoutPrice && (
                                                        <div className="flex items-end gap-1">
                                                            <input type={isVk ? "text" : "number"} inputMode={isVk ? "none" : undefined} data-vk-num={isVk ? "true" : undefined} autoComplete="off" min={1} max={stackCount} placeholder="Кол-во"
                                                            ...
                                                            <Button size="md" variant="secondary" onClick={() => { const q = partialQty[lot.id] || 1; handleBuyPartial(lot.id, q); }}
                                            ...
                                            {lot.sellerId === character?.id && (
                                                <Button size="md" variant="danger" onClick={() => handleCancel(lot.id)} disabled={!!lot.currentBidderId}>Снять лот</Button>
                                            )}
                                        </div>
                                    </div>
                                    <PriceChart item={item} />
