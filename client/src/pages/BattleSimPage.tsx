                ...
                <div>
                    {battles.map(b => (
                        <div key={b.num} className="mb-1">
                            ...
                            {openBattle === b.num && (
                                <div className="bg-[var(--color-bg-primary)] border-x border-b border-[var(--color-border-default)] rounded-b p-2 text-xs max-h-60 overflow-y-auto">
                                    {b.steps.map((s, i) => (
                                        <div key={i} className="py-0.5" style={{ color:
                                            s.type==='atk'?'#f9e2af':s.type==='dodge'?'#89b4fa':s.type==='counter'?'#f5c2e7':
                                            s.type==='crit'?'#f38ba8':s.type==='block'||s.type==='fullBlock'?'#a6e3a1':
                                            s.type==='stun'?'#cba6f7':s.type==='dmg'?'#f38ba8':s.type==='end'?'#f9e2af':
                                            s.type==='init'?'#b4befe':'#6c7086' }}>
                                            {s.message}
                                            {s.hA != null && s.hD != null && (
