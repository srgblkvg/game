...
export default function RecipeListItem({ recipe, onEdit, onDelete }: Props) {
  return (
    <div className="border-b border-[var(--color-border-light)] py-2 text-sm">
      ...
      <Button variant="primary" size="xs" className="mr-1 mt-1" onClick={onEdit}>Ред.</Button>
      <Button variant="danger" size="xs" className="mt-1" onClick={onDelete}>Удалить</Button>
    </div>
  );
}
