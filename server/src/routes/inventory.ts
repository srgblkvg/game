...
router.post('/character/equip', async (req: any, res) => {
    ...
    if (itemId === undefined || itemId === null) {
        ...
        await db.prepareRun('UPDATE users SET inventory = ?, equipment = ?, currentHp = ?, lastHpUpdate = ? WHERE id = ?')(JSON.stringify(inventory), JSON.stringify(equipment), newHp, now, userId);
...
// Разобрать предмет(ы)
router.post('/character/salvage', async (req: any, res) => {
    const userId = req.userId;
...
// Расширить инвентарь
router.post('/character/expand-inventory', async (req: any, res) => {
    const userId = req.userId;
    ...
    await db.prepareRun('UPDATE users SET money = money - ?, inventorySlots = inventorySlots + 1 WHERE id = ?')(price, userId);
