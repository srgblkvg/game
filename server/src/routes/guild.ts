...
router.post('/guild/war/attack', async (req, res) => {
    ...
    const attacker = await db.prepare('SELECT u.id, u.username, u.level, u.baseS, u.baseA, u.baseD, u.baseM, u.equipment, u.money, u.activeDrink, u.drinkUntil FROM users u WHERE u.id = ?').get(userId) as any;
    const defender = await db.prepare('SELECT u.id, u.username, u.level, u.baseS, u.baseA, u.baseD, u.baseM, u.equipment, u.money, u.activeDrink, u.drinkUntil FROM users u WHERE u.id = ?').get(targetId) as any;
    ...
    const { enriched: aEnriched } = await enrichEquipment(db, aEquip);
    const { enriched: dEnriched } = await enrichEquipment(db, dEquip);
    ...
    await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(war.id, userId, targetId, myGuildId, enemyGuildId, won ? 1 : 0, JSON.stringify(steps));
