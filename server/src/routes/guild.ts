...
router.post('/guild/create', async (req, res) => {
    ...
    const existing = await db.oneOrNone('SELECT id FROM guilds WHERE name = ?', [name.trim(]));
    if (existing) return res.status(400).json({ error: 'Гильдия с таким названием уже существует' });

    const alreadyInGuild = await db.oneOrNone('SELECT guildId FROM users WHERE id = ?', [userId]) as any;
    ...
    await db.none('INSERT INTO guild_members (guildId, userId, rank) VALUES (?, ?, ?)', [guildId, userId, 'leader']);
    await db.none('UPDATE users SET guildId = ? WHERE id = ?', [guildId, userId]);
...
router.get('/guild/my', async (req, res) => {
    ...
    let warInfo = null;
    if (war) {
        const attackerGuild = await db.oneOrNone('SELECT id, name FROM guilds WHERE id = ?', [war.attackerGuildId]) as any;
        const defenderGuild = await db.oneOrNone('SELECT id, name FROM guilds WHERE id = ?', [war.defenderGuildId]) as any;
        warInfo = {
            id: war.id,
            attackerGuild,
            defenderGuild,
...
router.get('/guild/list', async (req, res) => {
    ...
    // Дополнить war-инфой
    const result = (guilds as any[]).map(async (g: any) => {
        if (g.warId) {
            const war = await db.oneOrNone(`SELECT gw.*, a.name as attackerName, d.name as defenderName FROM guild_wars gw JOIN guilds a ON gw.attackerGuildId = a.id JOIN guilds d ON gw.defenderGuildId = d.id WHERE gw.id = ?`, [g.warId]) as any;
            if (war) {
...
router.get('/guild/requests', async (req, res) => {
    const userId = req.userId;
    const member = await db.oneOrNone('SELECT * FROM guild_members WHERE userId = ?', [userId]) as any;
...
router.get('/guild/chat', async (req, res) => {
    const userId = req.userId;
    const member = await db.oneOrNone('SELECT guildId FROM guild_members WHERE userId = ?', [userId]) as any;
...
router.post('/guild/chat', async (req, res) => {
    ...
    const guildId = member.guildId;
    const sender = await db.oneOrNone('SELECT username FROM users WHERE id = ?', [userId]) as any;
    const info = db.prepare(
    ...
    const members = await db.manyOrNone('SELECT userId FROM guild_members WHERE guildId = ?', [guildId]) as any[];
...
router.get('/guild/:id', async (req, res) => {
    ...
    const guild = await db.oneOrNone('SELECT g.*, u.username as leaderName FROM guilds g JOIN users u ON g.leaderId = u.id WHERE g.id = ?', [guildId]) as any;
...
router.post('/guild/join/:id', async (req, res) => {
    ...
    const guild = await db.oneOrNone('SELECT * FROM guilds WHERE id = ?', [guildId]) as any;
    if (!guild) return res.status(404).json({ error: 'Гильдия не найдена' });
    if (guild.joinType !== 'open') return res.status(400).json({ error: 'Гильдия доступна только по заявке или приглашению' });

    await db.none('INSERT INTO guild_members (guildId, userId, rank) VALUES (?, ?, ?)', [guildId, userId, 'member']);
    await db.none('UPDATE users SET guildId = ? WHERE id = ?', [guildId, userId]);
...
router.post('/guild/request/:id', async (req, res) => {
    ...
    await db.none('INSERT INTO guild_invites (guildId, userId, invitedBy) VALUES (?, ?, ?)', [guildId, userId, 0]);
    res.json({ success: true });
});
...
router.post('/guild/invite', async (req, res) => {
    ...
    const target = await db.oneOrNone('SELECT id, username, guildId FROM users WHERE id = ?', [targetId]) as any;
    ...
    const guild = await db.oneOrNone('SELECT name FROM guilds WHERE id = ?', [member.guildId]) as any;

    await db.none('INSERT INTO guild_invites (guildId, userId, invitedBy) VALUES (?, ?, ?)', [member.guildId, targetId, userId]);

    // ЛС с приглашением
    const inviter = await db.oneOrNone('SELECT username FROM users WHERE id = ?', [userId]) as any;
    const msg = `🏚️ ${inviter.username} приглашает вас в гильдию «${guild.name}». Нажмите чтобы принять.`;
    const info = await db.none('INSERT INTO chat_messages (senderId, targetId, content, item_data) VALUES (?, ?, ?, ?)', [0, targetId, msg, JSON.stringify({ type: 'guild_invite', guildId: member.guildId, guildName: guild.name }]));
...
router.post('/guild/invite/:id', async (req, res) => {
    ...
    const invite = await db.oneOrNone('SELECT * FROM guild_invites WHERE id = ? AND userId = ? AND status = ?', [inviteId, userId, 'pending']) as any;
    if (!invite) return res.status(404).json({ error: 'Приглашение не найдено' });
    ...
    if (accept) {
        ...
        await db.none('INSERT INTO guild_members (guildId, userId, rank) VALUES (?, ?, ?)', [invite.guildId, userId, 'member']);
        await db.none('UPDATE users SET guildId = ? WHERE id = ?', [invite.guildId, userId]);
        await db.none('UPDATE guild_invites SET status = ? WHERE id = ?', ['accepted', inviteId]);
    } else {
        await db.none('UPDATE guild_invites SET status = ? WHERE id = ?', ['declined', inviteId]);
    }
...
router.post('/guild/handle-request', async (req, res) => {
    ...
    if (accept) {
        const targetGuild = await db.oneOrNone('SELECT guildId FROM users WHERE id = ?', [invite.userId]) as any;
        if (targetGuild?.guildId) return res.status(400).json({ error: 'Игрок уже в гильдии' });

        await db.none('INSERT INTO guild_members (guildId, userId, rank) VALUES (?, ?, ?)', [member.guildId, invite.userId, 'member']);
        await db.none('UPDATE users SET guildId = ? WHERE id = ?', [member.guildId, invite.userId]);
        await db.none('UPDATE guild_invites SET status = ? WHERE id = ?', ['accepted', requestId]);
    } else {
        await db.none('UPDATE guild_invites SET status = ? WHERE id = ?', ['declined', requestId]);
    }
...
router.post('/guild/leave', async (req, res) => {
    ...
    if (member.rank === 'leader') {
        const otherMembers = await db.oneOrNone('SELECT COUNT(*) as cnt FROM guild_members WHERE guildId = ? AND userId != ?', [member.guildId, userId]) as any;
        if (otherMembers.cnt > 0) return res.status(400).json({ error: 'Передайте лидерство другому члену перед выходом' });
        await db.none('DELETE FROM guilds WHERE id = ?', [member.guildId]);
    }

    await db.none('DELETE FROM guild_members WHERE guildId = ? AND userId = ?', [member.guildId, userId]);
    await db.none('UPDATE users SET guildId = NULL WHERE id = ?', [userId]);
...
router.post('/guild/kick', async (req, res) => {
    ...
    const actor = await db.oneOrNone('SELECT * FROM guild_members WHERE userId = ?', [userId]) as any;
    if (!actor || (actor.rank !== 'leader' && actor.rank !== 'officer')) return res.status(400).json({ error: 'Нет прав' });

    const target = await db.oneOrNone('SELECT * FROM guild_members WHERE guildId = ? AND userId = ?', [actor.guildId, targetId]) as any;
    ...
    await db.none('DELETE FROM guild_members WHERE guildId = ? AND userId = ?', [actor.guildId, targetId]);
    await db.none('UPDATE users SET guildId = NULL WHERE id = ?', [targetId]);
...
router.post('/guild/role', async (req, res) => {
    ...
    await db.none('UPDATE guild_members SET rank = ? WHERE guildId = ? AND userId = ?', [rank, actor.guildId, targetId]);
    res.json({ success: true });
});
...
router.post('/guild/treasury/deposit', async (req, res) => {
    ...
    const tx = db.tx(async (t) => {
        await db.none('UPDATE users SET money = money - ? WHERE id = ?', [amount, userId]);
        await db.none('UPDATE guilds SET treasury = treasury + ? WHERE id = ?', [amount, member.guildId]);
        await db.none('INSERT INTO guild_treasury_log (guildId, userId, amount) VALUES (?, ?, ?)', [member.guildId, userId, amount]);
        return await db.oneOrNone('SELECT treasury FROM guilds WHERE id = ?', [member.guildId]) as any;
    });
...
router.get('/guild/treasury/history', async (req, res) => {
    ...
    const total = (await db.oneOrNone(`SELECT COUNT(*) as cnt FROM guild_treasury_log l JOIN users u ON l.userId = u.id ${whereClause}`, [...params]) as any).cnt;
    ...
    const treasury = (await db.oneOrNone('SELECT treasury FROM guilds WHERE id = ?', [member.guildId]) as any)?.treasury || 0;
...
async function isGuildAtWar(guildId: number): any | null {
    ...
    for (const war of expiredWars) {
        ...
        if (winnerId && loserId) {
            const loserTreasury = (await db.oneOrNone('SELECT treasury FROM guilds WHERE id = ?', [loserId]) as any)?.treasury || 0;
            if (loserTreasury > 0) {
                await db.none('UPDATE guilds SET treasury = treasury + ? WHERE id = ?', [loserTreasury, winnerId]);
                await db.none('UPDATE guilds SET treasury = 0 WHERE id = ?', [loserId]);
                // Запись в лог казны
                await db.none('INSERT INTO guild_treasury_log (guildId, userId, amount, type) VALUES (?, ?, ?, ?)', [winnerId, 0, loserTreasury, 'war_win']);
                await db.none('INSERT INTO guild_treasury_log (guildId, userId, amount, type) VALUES (?, ?, ?, ?)', [loserId, 0, -loserTreasury, 'war_loss']);
            }
        }
...
router.post('/guild/war/declare', async (req, res) => {
    ...
    const targetGuild = await db.oneOrNone('SELECT * FROM guilds WHERE id = ?', [targetGuildId]) as any;
    ...
    const myGuild = await db.oneOrNone('SELECT name FROM guilds WHERE id = ?', [myGuildId]) as any;
...
router.post('/guild/war/respond', async (req, res) => {
    ...
    if (accept) {
        ...
        await db.none('UPDATE guild_wars SET status = ?, acceptedAt = ?, expiresAt = ? WHERE id = ?', ['active', now, newExpiresAt, war.id]);
        res.json({ success: true, message: 'Война принята! Казна заморожена на 24 часа.' });
    } else {
        await db.none('UPDATE guild_wars SET status = ?, endedAt = ? WHERE id = ?', ['cancelled', now, war.id]);
...
router.get('/guild/war/details', async (req, res) => {
    ...
    const myGuild = await db.oneOrNone('SELECT id, name FROM guilds WHERE id = ?', [myGuildId]) as any;
    const enemyGuild = await db.oneOrNone('SELECT id, name FROM guilds WHERE id = ?', [enemyGuildId]) as any;

    // Участники моей гильдии (только кто был в гильдии на момент объявления войны)
    const myMembers = await db.manyOrNone(`
    ...
    const now = new Date().toISOString().replace('T', ' ').slice(0, 19);
    const enemyMembers = await db.manyOrNone(`
        SELECT u.id, u.username, u.level,
    ...
    // Мои атаки
    const myAttacks = await db.manyOrNone(`
    ...
    const allAttacks = await db.manyOrNone(`
...
router.post('/guild/war/attack', async (req, res) => {
    ...
    const targetMember = await db.oneOrNone('SELECT * FROM guild_members WHERE guildId = ? AND userId = ?', [enemyGuildId, targetId]) as any;
    ...
    const attacker = await db.oneOrNone('SELECT u.id, u.username, u.level, u.baseS, u.baseA, u.baseD, u.baseM, u.equipment, u.money FROM users u WHERE u.id = ?', [userId]) as any;
    const defender = await db.oneOrNone('SELECT u.id, u.username, u.level, u.baseS, u.baseA, u.baseD, u.baseM, u.equipment, u.money FROM users u WHERE u.id = ?', [targetId]) as any;

    const { currentStats } = require('../game/stats');
    const { getBaseStats } = require('../db/helpers');

    const aCollCnt = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM collections WHERE userId = ?', [userId]) as any).cnt || 0;
    const dCollCnt = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM collections WHERE userId = ?', [targetId]) as any).cnt || 0;
    ...
    if (won) {
        ...
        await db.none(`UPDATE guild_wars SET ${scoreField} = ${scoreField} + 1 WHERE id = ?`, [war.id]);
    }
...
router.post('/guild/tax-rate', async (req, res) => {
    ...
    await db.none('UPDATE guilds SET taxRate = ? WHERE id = ?', [taxRate, member.guildId]);
