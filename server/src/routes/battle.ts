...
router.post('/battle', async (req, res) => {
    ...
    const attacker = await db.oneOrNone('SELECT u.id, u.username, u.level, u.exp, u.currentHp, u.elo, u.seasonWins, u.seasonLosses, u.equipment, u.baseS, u.baseA, u.baseD, u.baseM, u.money, u.inventorySlots, u.lastAttackTime, u.premiumUntil, g.name as guildName FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id = ?', [userId]) as any;
    ...
    let defender: any;
    if (opponentId) {
        defender = await db.oneOrNone('SELECT u.id, u.username, u.level, u.exp, u.currentHp, u.elo, u.seasonWins, u.seasonLosses, u.equipment, u.baseS, u.baseA, u.baseD, u.baseM, u.money, u.inventorySlots, u.protectionUntil, u.roomType, u.roomUntil, u.lastHpUpdate, g.name as guildName FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id = ?', [opponentId]);
        if (!defender || defender.id == userId) return res.status(400).json({ error: 'Invalid opponent' });
    } else {
        const others = await db.manyOrNone('SELECT u.id, u.username, u.level, u.exp, u.currentHp, u.elo, u.seasonWins, u.seasonLosses, u.equipment, u.baseS, u.baseA, u.baseD, u.baseM, u.money, u.inventorySlots, u.protectionUntil, u.roomType, u.roomUntil, u.lastHpUpdate, g.name as guildName FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id != ? AND u.id > 0 AND (u.protectionUntil IS NULL OR u.protectionUntil < ?)', [userId, now]) as any[];
    ...
    const dCollCnt = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM collections WHERE userId = ?', [defender.id]) as any).cnt || 0;
    ...
    const attackerData = {
        ...
        collectionBonus: (await db.oneOrNone('SELECT COUNT(*) as cnt FROM collections WHERE userId = ?', [attacker.id]) as any).cnt || 0,
    };
    const defenderData = {
        id: defender.id,
    ...
    if (moneyStolen > 0) {
        if (attackerWins) {
            await db.none('UPDATE users SET money = money - ? WHERE id = ?', [moneyStolen, defender.id]);
            await db.none('UPDATE users SET money = money + ? WHERE id = ?', [moneyStolen, attacker.id]);
        } else {
            await db.none('UPDATE users SET money = money - ? WHERE id = ?', [moneyStolen, attacker.id]);
            await db.none('UPDATE users SET money = money + ? WHERE id = ?', [moneyStolen, defender.id]);
        }
    ...
    const updatedAttacker = await db.oneOrNone('SELECT money FROM users WHERE id = ?', [userId]) as any;
...
router.get('/battles', async (req, res) => {
    ...
    const battles = await db.manyOrNone(`
    SELECT b.*, 
...
adminRouter.get('/battles', async (req, res) => {
    ...
    const total = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM battles') as any).cnt;
    const battles = await db.manyOrNone(`
