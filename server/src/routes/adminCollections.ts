...
router.get('/collection-sets', async (req, res) => {
    const sets = await db.manyOrNone('SELECT * FROM collection_sets ORDER BY sort_order');
...
router.put('/collection-sets/:id', async (req, res) => {
    ...
    const existing = await db.oneOrNone('SELECT id FROM collection_sets WHERE id = ?', [setId]);
    ...
    if (items !== undefined) {
        await db.none('DELETE FROM collection_set_items WHERE set_id = ?', [setId]);
...
router.delete('/collection-sets/:id', async (req, res) => {
    const setId = Number(req.params.id);
    await db.none('DELETE FROM collection_set_items WHERE set_id = ?', [setId]);
    await db.none('DELETE FROM collection_sets WHERE id = ?', [setId]);
    res.json({ success: true });
});
