...
router.get('/rating', async (req, res) => {
    ...
    const total = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM users WHERE id > 0') as any).cnt;
...
router.get('/my-position', async (req, res) => {
    ...
    const user = await db.oneOrNone('SELECT elo FROM users WHERE id = ?', [userId]) as any;
