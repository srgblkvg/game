...
router.get('/pve-battles', authMiddleware, async async (req, res) => {
...
router.get('/tournament-history', authMiddleware, async async (req, res) => {
    const userId = req.userId;
    const limit = parseInt(req.query.limit as string) || 50;
...
// История квестов
router.get('/quest-history', authMiddleware, async async (req, res) => {
    const userId = req.userId;
...
router.post('/log/error', async async (req, res) => {
