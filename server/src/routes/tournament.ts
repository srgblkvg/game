...
async function generateBracket(tournamentId: number) {
    ...
    if (participants.length < 2) {
        ...
        await db.prepare('UPDATE tournaments SET status = ?, completedAt = ? WHERE id = ?').run('cancelled', new Date().toISOString(), tournamentId);
        return;
    }
...
async function finishTournament(tournamentId: number) {
    ...
    await db.prepare('UPDATE tournament_participants SET snapshotStats = ? WHERE tournamentId = ? AND userId = ?')
    ...
    await db.prepare('UPDATE tournaments SET status = ?, completedAt = ? WHERE id = ?').run('completed', new Date().toISOString(), tournamentId);
    ...
    for (const p of allParts) {
        ...
        await db.prepare('UPDATE users SET tournamentElo = MAX(100, tournamentElo + ?) WHERE id = ?')
            .run(delta, p.userId);
    }
}
...
async function autoAdvance(tournamentId: number) {
    ...
    if (t.status === 'registration' && now >= t.registrationEnd - 300 && now < t.registrationEnd) {
        const already = await db.prepare("SELECT id FROM chat_messages WHERE senderId = 0 AND content LIKE '%регистрация закроется через%' AND createdAt > NOW() - INTERVAL '5 minutes'").get();
        if (!already) {
            ...
            await db.prepare('INSERT INTO chat_messages (senderId, targetId, content) VALUES (?, ?, ?)')
...
router.post('/tournament/register', async (req, res) => {
    ...
    await db.prepare('INSERT INTO tournament_participants (tournamentId, userId, goldenTicket) VALUES (?, ?, ?)')
