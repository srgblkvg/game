...
router.post('/register', async (req, res) => {
    ...
    const existing = await db.oneOrNone('SELECT id, username FROM users WHERE username = ? OR email = ?', [username, email]) as any;
    if (existing) {
    ...
    await db.none(`INSERT INTO users (username, passwordHash, email, emailCode, emailCodeExpires, currentHp, lastHpUpdate, level, gender)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, 'male')`, [username, passwordHash, email, code, codeExpires, startHp, now]);
    ...
    if (!sent) {
        ...
        await db.none('UPDATE users SET emailCode = NULL, emailCodeExpires = 0 WHERE email = ?', [email]);
...
router.post('/verify-email', async (req, res) => {
    ...
    const user: any = await db.oneOrNone('SELECT id, username, emailCode, emailCodeExpires, emailVerified FROM users WHERE email = ?', [email]);
    ...
    await db.none('UPDATE users SET emailVerified = 1, emailCode = NULL, emailCodeExpires = 0, lastLoginAt = ? WHERE id = ?', [now, user.id]);
...
router.post('/resend-code', async (req, res) => {
    ...
    if (authHeader?.startsWith('Bearer ')) {
        try {
            ...
            if (decoded.isGuest && decoded.userId) {
                const guestUser: any = await db.oneOrNone('SELECT id FROM users WHERE id = ?', [decoded.userId]);
                if (guestUser) {
                    // Проверяем, не занят ли email другим пользователем
                    const emailTaken = await db.oneOrNone('SELECT id FROM users WHERE email = ? AND id != ?', [email, decoded.userId]);
                    if (emailTaken) return res.status(400).json({ error: 'Этот email уже используется' });

                    const code = generateCode();
                    const codeExpires = now + 600;
                    await db.none('UPDATE users SET email = ?, emailCode = ?, emailCodeExpires = ? WHERE id = ?', [email, code, codeExpires, decoded.userId]);
    ...
    const user: any = await db.oneOrNone('SELECT id, emailVerified FROM users WHERE email = ?', [email]);
    if (!user) return res.status(400).json({ error: 'Email не найден' });
    if (user.emailVerified) return res.status(400).json({ error: 'Email уже подтверждён' });

    const code = generateCode();
    const codeExpires = now + 600;
    await db.none('UPDATE users SET emailCode = ?, emailCodeExpires = ? WHERE id = ?', [code, codeExpires, user.id]);
...
router.post('/guest', async (req, res) => {
    ...
    await db.none(`INSERT INTO users (username, passwordHash, currentHp, lastHpUpdate, level, gender, isGuest, emailVerified, exp, money)
        VALUES (?, '', ?, ?, 1, 'male', 1, 1, 0, 0)`, [guestId, startHp, now]);

    const user: any = await db.oneOrNone('SELECT id FROM users WHERE username = ?', [guestId]);
    ...
    if (req.ip) {
        await db.none('INSERT INTO login_logs (userId, ip) VALUES (?, ?)', [user.id, req.ip]);
    }
...
router.post('/login', async (req, res) => {
    ...
    const userRow: any = await db.oneOrNone('SELECT id, passwordHash, failedLogins, lockedUntil, bannedUntil FROM users WHERE username = ? OR email = ?', [login, login]);
    ...
    const admin: any = await db.oneOrNone('SELECT * FROM admins WHERE username = ?', [login]);
    ...
    if (!userRow || !bcrypt.compareSync(password, userRow.passwordHash)) {
        ...
        if (userRow) {
          ...
          await db.none('UPDATE users SET failedLogins = ?, lockedUntil = ? WHERE id = ?', [newFailed, lockedUntil, userRow.id]);
    ...
    const emailUser: any = await db.oneOrNone('SELECT email, emailVerified FROM users WHERE id = ?', [userRow.id]);
    ...
    await db.none('UPDATE users SET failedLogins = 0, lockedUntil = 0, lastLoginAt = ? WHERE id = ?', [now, userRow.id]);
    auditLoginSuccess(login, userRow.id, req.ip);

    // Декай рейтинга и проверка сезона
    const ratingUser: any = await db.oneOrNone('SELECT elo, lastPvpTime FROM users WHERE id = ?', [userRow.id]);
    if (ratingUser) {
    ...
    // Логируем IP
    if (req.ip) {
        await db.none('INSERT INTO login_logs (userId, ip) VALUES (?, ?)', [userRow.id, req.ip]);
    }

    const token = jwt.sign({ userId: userRow.id, role: 'player', jti: crypto.randomUUID() }, JWT_SECRET, { expiresIn: '30d' });
    const fullUser: any = await db.oneOrNone('SELECT gender FROM users WHERE id = ?', [userRow.id]);
