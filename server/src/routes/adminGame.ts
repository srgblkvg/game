...
router.post('/actions', async (req, res) => {
    ...
    await db.prepare('INSERT INTO actions_config (section, title, subtitle, icon, bg_image, path, cost, sort_order) VALUES (?,?,?,?,?,?,?,?)').run(section || 'world', title, subtitle || '', icon || '', bg_image || null, path || null, cost || 0, sort_order || 0);
    res.json({ success: true });
});

router.put('/actions/:id', async (req, res) => {
    const { section, title, subtitle, icon, bg_image, path, cost, sort_order } = req.body;
    await db.prepare('UPDATE actions_config SET section=?,title=?,subtitle=?,icon=?,bg_image=?,path=?,cost=?,sort_order=? WHERE id=?').run(section, title, subtitle, icon, bg_image || null, path || null, cost, sort_order, req.params.id);
    res.json({ success: true });
});
...
router.put('/mobs/:id', async (req, res) => {
    ...
    await db.prepare(`UPDATE mobs SET name=?,level=?,hp=?,atk=?,agi=?,def=?,mst=?,xp=?,gold_min=?,gold_max=?,
        ...
        WHERE id=?`).run(name, level, hp, atk, agi, def, mst, xp, gold_min, gold_max,
...
router.post('/floors', async (req, res) => {
    ...
    await db.prepare('INSERT INTO floors (name, background, sort_order) VALUES (?,?,?)').run(name, background || null, sort_order || 0);
    res.json({ success: true });
});

router.put('/floors/:id', async (req, res) => {
    const { name, background, sort_order } = req.body;
    await db.prepare('UPDATE floors SET name=?, background=?, sort_order=? WHERE id=?').run(name, background || null, sort_order || 0, req.params.id);
    res.json({ success: true });
});
