...
router.post('/ban-chat', async (req, res) => {
    ...
    await db.run('UPDATE users SET chatBannedUntil = ? WHERE id = ?', [banUntil, userId]);
...
router.post('/unban', async (req, res) => {
    ...
    await db.run('UPDATE users SET chatBannedUntil = 0 WHERE id = ?', [userId]);
    res.json({ success: true });
});
...
router.post('/system-message', async (req, res) => {
    ...
    const info = await db.run('INSERT INTO chat_messages (senderId, targetId, content) VALUES (?, ?, ?)', [0, null, content]);
    const msg = {
