...
router.post('/craft/execute', async (req, res) => {
    ...
    const recipe = await db.oneOrNone('SELECT * FROM craft_recipes WHERE id = ?', [Number(recipe_id])) as any;
  ...
  `, [recipe.id]) as any[];
    ...
    if (success) {
        if (recipe.result_type === 'item') {
            const resultItem = await db.oneOrNone(`
      ...
      `, [recipe.result_id]) as any;
            ...
            const randomItem = await db.oneOrNone(`
                SELECT i.*, r.display_name as rarity_display, r.color as rarity_color
                FROM items i
                JOIN rarities r ON i.rarity_id = r.id
                WHERE i.rarity_id = ?
            ...
            const resultCraftItem = await db.oneOrNone(`
      ...
      `, [recipe.result_id]) as any;
        ...
        await db.none('UPDATE users SET inventory = ?, money = ?, craftCount = craftCount + 1, craftCreated = craftCreated + 1 WHERE id = ?', [JSON.stringify(newInventory]), newMoney, userId);
        return res.json({ success: true, inventory: newInventory, moneyAfter: newMoney, message: 'Предмет создан!' });
    } else {
        await db.none('UPDATE users SET inventory = ?, money = ?, craftBroken = craftBroken + 1 WHERE id = ?', [JSON.stringify(newInventory]), newMoney, userId);
...
router.get('/craft/upgrade-info/:level/:rarity', async (req, res) => {
    ...
    const data = await db.oneOrNone('SELECT chance, money_cost FROM upgrade_chances WHERE level = ? AND rarity_id = ?', [level, rarity]) as any;
...
router.post('/craft/upgrade', async (req, res) => {
    ...
    const upgradeData = await db.oneOrNone('SELECT chance, money_cost FROM upgrade_chances WHERE level = ? AND rarity_id = ?', [targetLevel, stoneSlot.rarity_id]) as any;
    if (!upgradeData) {
    ...
    if (success) {
        ...
        if (ratingBonus > 0) {
            ...
            await db.none('UPDATE users SET money = ?, inventory = ?, elo = ?, pveRating = pveRating + ?, craftCount = craftCount + 1, craftUpgraded = craftUpgraded + 1 WHERE id = ?', [newMoney, JSON.stringify(newInventory]), newElo, ratingBonus, userId);
        ...
        await db.none('UPDATE users SET inventory = ?, money = ?, craftCount = craftCount + 1, craftUpgraded = craftUpgraded + 1 WHERE id = ?', [JSON.stringify(newInventory]), newMoney, userId);
        ...
        if (itemIdx !== -1) {
            ...
            const craftItem = await db.oneOrNone(`
