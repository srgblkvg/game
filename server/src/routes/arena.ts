...
router.get('/arena/opponent', async (req, res) => {
    ...
    const user: any = await db.oneOrNone('SELECT u.id, u.username, u.level, u.elo, u.seasonWins, u.seasonLosses, u.equipment, u.baseS, u.baseA, u.baseD, u.baseM, u.inventorySlots, u.lastAttackTime, u.money, u.arenaOpponentId, g.name as guildName, u.guildId FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id = ?', [userId]);
    ...
    if (!change && user.arenaOpponentId) {
        ...
        if (saved) {
            ...
            if (matchesDifficulty) {
                ...
                const savedCollCnt = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM collections WHERE userId = ?', [saved.id]) as any).cnt || 0;
            ...
            await db.none('UPDATE users SET money = money - 10 WHERE id = ?', [userId]);
            user.money -= 10;
        }
    ...
    await db.none('UPDATE users SET arenaOpponentId = ? WHERE id = ?', [opponent.id, userId]);
    ...
    const oppCollCnt = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM collections WHERE userId = ?', [opponent.id]) as any).cnt || 0;
