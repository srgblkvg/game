...
router.get('/orders/my', async (req, res) => {
    ...
    const member = await db.oneOrNone('SELECT * FROM order_members WHERE userId = ?', [userId]) as any;
...
router.post('/orders/create', async (req, res) => {
    ...
    const existing = await db.oneOrNone('SELECT id FROM orders WHERE name = ?', [name]);
    if (existing) return res.status(400).json({ error: 'Имя занято' });

    const memberCheck = await db.oneOrNone('SELECT id FROM order_members WHERE userId = ?', [userId]);
    if (memberCheck) return res.status(400).json({ error: 'Вы уже в ордене' });

    const now = Math.floor(Date.now() / 1000);
    await db.none('UPDATE users SET money = money - 5000 WHERE id = ?', [userId]);
    const result = await db.none('INSERT INTO orders (name, masterId, createdAt) VALUES (?, ?, ?)', [name, userId, now]);
    await db.none('INSERT INTO order_members (orderId, userId, rank, joinedAt) VALUES (?, ?, ?, ?)', [result.lastInsertRowid, userId, 'master', now]);
...
router.post('/orders/join', async (req, res) => {
    ...
    const order = await db.oneOrNone('SELECT * FROM orders WHERE id = ?', [orderId]) as any;
    if (!order) return res.status(404).json({ error: 'Орден не найден' });

    const count = (await db.oneOrNone('SELECT COUNT(*) as cnt FROM order_members WHERE orderId = ?', [orderId]) as any).cnt;
    const maxMembers = Math.min(25, 10 + order.level);
    if (count >= maxMembers) return res.status(400).json({ error: 'Орден заполнен' });

    const now = Math.floor(Date.now() / 1000);
    await db.none('INSERT INTO order_members (orderId, userId, rank, joinedAt) VALUES (?, ?, ?, ?)', [orderId, userId, 'recruit', now]);
...
router.post('/orders/donate', async (req, res) => {
    ...
    await db.none('UPDATE orders SET treasury = treasury + ?, exp = exp + ? WHERE id = ?', [amount, Math.floor(amount / 100]), member.orderId);
    res.json({ success: true });
});
...
router.post('/orders/promote', async (req, res) => {
    ...
    const myMembership = await db.oneOrNone('SELECT * FROM order_members WHERE userId = ?', [userId]) as any;
    if (!myMembership || !['master', 'officer'].includes(myMembership.rank)) return res.status(403).json({ error: 'Нет прав' });

    const target = await db.oneOrNone('SELECT * FROM order_members WHERE userId = ? AND orderId = ?', [targetUserId, myMembership.orderId]) as any;
    ...
    await db.none('UPDATE order_members SET rank = ? WHERE userId = ?', [newRank, targetUserId]);
    res.json({ success: true });
});
...
router.post('/orders/kick', async (req, res) => {
    ...
    await db.none('DELETE FROM order_members WHERE userId = ?', [targetUserId]);
    res.json({ success: true });
});
