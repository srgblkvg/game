...
// Мой орден
router.get('/orders/my', async (req: any, res) => {
...
// Вступить в орден
router.post('/orders/join', async (req: any, res) => {
    const userId = req.userId;
...
// Покинуть орден
router.post('/orders/leave', async (req: any, res) => {
    const userId = req.userId;
    const member = await db.prepareGet('SELECT * FROM order_members WHERE userId = ?')(userId) as any;
...
router.post('/orders/donate', async (req: any, res) => {
    const userId = req.userId;
...
router.post('/orders/promote', async (req: any, res) => {
    const userId = req.userId;
...
// Исключить участника
router.post('/orders/kick', async (req: any, res) => {
    const userId = req.userId;
