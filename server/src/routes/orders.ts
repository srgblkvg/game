...
router.get('/orders/my', async (req, res) => {
    ...
    const member = await db.one('SELECT * FROM order_members WHERE userId = ?', [userId]) as any;
...
router.post('/orders/create', async (req, res) => {
    ...
    const memberCheck = await db.one('SELECT id FROM order_members WHERE userId = ?', [userId]);
...
router.post('/orders/join', async (req, res) => {
    ...
    const order = await db.one('SELECT * FROM orders WHERE id = ?', [orderId]) as any;
    if (!order) return res.status(404).json({ error: 'Орден не найден' });

    const count = (await db.one('SELECT COUNT(*) as cnt FROM order_members WHERE orderId = ?', [orderId]) as any).cnt;
    const maxMembers = Math.min(25, 10 + order.level);
    if (count >= maxMembers) return res.status(400).json({ error: 'Орден заполнен' });

    const now = Math.floor(Date.now() / 1000);
    await db.run('INSERT INTO order_members (orderId, userId, rank, joinedAt) VALUES (?, ?, ?, ?)', [orderId, userId, 'recruit', now]);
...
router.post('/orders/donate', async (req, res) => {
    ...
    await db.run('UPDATE orders SET treasury = treasury + ?, exp = exp + ? WHERE id = ?', [amount, Math.floor(amount / 100), member.orderId]);
    res.json({ success: true });
});
...
router.post('/orders/promote', async (req, res) => {
    ...
    const myMembership = await db.one('SELECT * FROM order_members WHERE userId = ?', [userId]) as any;
    if (!myMembership || !['master', 'officer'].includes(myMembership.rank)) return res.status(403).json({ error: 'Нет прав' });

    const target = await db.one('SELECT * FROM order_members WHERE userId = ? AND orderId = ?', [targetUserId, myMembership.orderId]) as any;
    ...
    await db.run('UPDATE order_members SET rank = ? WHERE userId = ?', [newRank, targetUserId]);
    res.json({ success: true });
});
...
router.post('/orders/kick', async (req, res) => {
    ...
    await db.run('DELETE FROM order_members WHERE userId = ?', [targetUserId]);
    res.json({ success: true });
});
