...
router.post('/craft-items', async (req, res) => {
    ...
    await db.none('INSERT INTO craft_items (name, rarity_id, description, type, image) VALUES (?, ?, ?, ?, ?)', [name, rarity_id, description || '', type || 'craft', image || null]);
    res.json({ success: true });
});

router.put('/craft-items/:id', async (req, res) => {
    const { name, rarity_id, description, type, image } = req.body;
    await db.none('UPDATE craft_items SET name=?, rarity_id=?, description=?, type=?, image=? WHERE id=?', [name, rarity_id, description, type || 'craft', image || null, req.params.id]);
    res.json({ success: true });
});

router.delete('/craft-items/:id', async (req, res) => {
    await db.none('DELETE FROM craft_items WHERE id=?', [req.params.id]);
    res.json({ success: true });
});

// ==================== Рецепты (craft_recipes) ====================
router.get('/recipes', async (req, res) => {
    const recipes = await db.manyOrNone('SELECT * FROM craft_recipes ORDER BY id') as any[];
    for (const recipe of recipes) {
        recipe.ingredients = await db.manyOrNone(`
        ...
        if (recipe.result_type === 'item') {
            recipe.result = await db.oneOrNone(`
        ...
        recipe.category = await db.oneOrNone('SELECT * FROM craft_recipe_categories WHERE id = ?', [recipe.category_id]) || null;
    }
    res.json(recipes);
});
...
router.put('/recipes/:id', async (req, res) => {
    ...
    await db.none('DELETE FROM craft_recipe_ingredients WHERE recipe_id=?', [req.params.id]);
    if (ingredients && Array.isArray(ingredients)) {
        const stmt = db.prepare('INSERT INTO craft_recipe_ingredients (recipe_id, craft_item_id, quantity) VALUES (?, ?, ?)');
        for (const ing of ingredients) {
...
router.delete('/recipes/:id', async (req, res) => {
    await db.none('DELETE FROM craft_recipes WHERE id=?', [req.params.id]);
    await db.none('DELETE FROM craft_recipe_ingredients WHERE recipe_id=?', [req.params.id]);
    res.json({ success: true });
});
...
router.get('/recipe-categories', async (req, res) => {
    const cats = await db.manyOrNone('SELECT * FROM craft_recipe_categories ORDER BY sort_order, id');
    res.json(cats);
});
...
router.post('/recipe-categories', async (req, res) => {
    ...
    await db.none('INSERT INTO craft_recipe_categories (name, sort_order) VALUES (?, ?)', [name, sort_order || 0]);
    res.json({ success: true });
});

router.put('/recipe-categories/:id', async (req, res) => {
    const { name, sort_order } = req.body;
    await db.none('UPDATE craft_recipe_categories SET name=?, sort_order=? WHERE id=?', [name, sort_order || 0, req.params.id]);
    res.json({ success: true });
});

router.delete('/recipe-categories/:id', async (req, res) => {
    await db.none('DELETE FROM craft_recipe_categories WHERE id=?', [req.params.id]);
    res.json({ success: true });
});

// ==================== Шансы улучшения (upgrade_chances) ====================
router.get('/upgrade-chances', async (req, res) => {
    const chances = await db.manyOrNone('SELECT * FROM upgrade_chances ORDER BY rarity_id, level');
    res.json(chances);
});
...
router.post('/upgrade-chances', async (req, res) => {
    ...
    await db.none('INSERT OR REPLACE INTO upgrade_chances (level, rarity_id, chance, money_cost) VALUES (?, ?, ?, ?)', [level, rId, chance, money_cost]);
    res.json({ success: true });
});

router.put('/upgrade-chances/:level/:rarity_id', async (req, res) => {
    const { chance, money_cost } = req.body;
    await db.none('UPDATE upgrade_chances SET chance=?, money_cost=? WHERE level=? AND rarity_id=?', [chance, money_cost, req.params.level, req.params.rarity_id]);
    res.json({ success: true });
});

router.delete('/upgrade-chances/:level/:rarity_id', async (req, res) => {
    await db.none('DELETE FROM upgrade_chances WHERE level=? AND rarity_id=?', [req.params.level, req.params.rarity_id]);
    res.json({ success: true });
});
