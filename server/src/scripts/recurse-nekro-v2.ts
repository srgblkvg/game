...
function curseS(rank: number): { curseStat: string; curseValue: number; curseRank: number; curseName: string; curseColor: string } {
...
async function main() {
    ...
    await db.run('DELETE FROM overflow_storage WHERE userid = ?', [userId]);
    ...
    // Сохраняем в слот 2
    await db.run('UPDATE users SET equipment_2 = ?::jsonb WHERE id = ?', [JSON.stringify(set2), userId]);
