...
function rollCurse() {
    ...
    const stats = ['s', 'm'] as const;  // only S and M
...
async function main() {
    const uid = 1;
    const user = await db.one('SELECT equipment FROM users WHERE id = ?', [uid]) as any;
    ...
    for (const [slot, item] of Object.entries(equipment)) {
        ...
        console.log(`${slot}: ${(item as any).name} → +${curse.value} ${statName} (ранг ${curse.name})`);
    }

    await db.run('UPDATE users SET equipment = ? WHERE id = ?', [JSON.stringify(equipment), uid]);
    console.log('Готово.');
}
