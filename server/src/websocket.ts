...
export async function setupWebSocket(server: any) {
  ...
  wss.on('connection', async (ws: WebSocket, req) => {
    ...
    if (decoded.role === 'admin') {
      const admin = await db.oneOrNone('SELECT id, username FROM admins WHERE id = ?', [decoded.adminId]) as any;
      if (!admin) {
    ...
    const user = await db.oneOrNone('SELECT u.id, u.username, u.level, u.chatBannedUntil, u.isGuest, g.name as guildName, u.guildId FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id = ?', [userId]) as any;
    if (!user) {
    ...
    ws.on('message', async (raw) => {
      ...
      const currentUser = await db.oneOrNone('SELECT chatBannedUntil FROM users WHERE id = ?', [userId]) as any;
      ...
      if (data.type === 'itemLink') {
        ...
        if (!item) {
          ...
          if (!itemId) return;
          const currentUser = await db.oneOrNone('SELECT inventory FROM users WHERE id = ?', [userId]) as any;
      ...
      if (data.type === 'public') {
        ...
        if (content.startsWith('/w ')) {
          ...
          const targetUser = await db.oneOrNone('SELECT id FROM users WHERE LOWER(username) = ?', [targetName]) as any;
          if (!targetUser) {
