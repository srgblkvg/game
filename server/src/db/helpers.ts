...
export async function getItemDataStmt(db: DB) {
  if (!_getItemData) {
...
export async function getUserById(db: DB, userId: number) {
  return await db.oneOrNone('SELECT u.*, g.name as guildName FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id = ?', [userId]) as any;
}
...
export async function enrichEquipment(db: DB, equipment: Record<string, any>): { enriched: Record<string, any>; changed: boolean } {
...
export async function transferMoney(db: DB, fromUserId: number, toUserId: number, amount: number) {
  ...
  await db.none('UPDATE users SET money = money + ? WHERE id = ?', [amount, toUserId]);
  return true;
}
...
export async function spendMoney(db: DB, userId: number, amount: number): boolean {
  const result = await db.none('UPDATE users SET money = money - ? WHERE id = ? AND money >= ?', [amount, userId, amount]);
...
export async function collectGuildTax(db: DB, userId: number, income: number, source: string): number {
  ...
  const member = await db.oneOrNone('SELECT gm.guildId, g.taxRate FROM guild_members gm JOIN guilds g ON gm.guildId = g.id WHERE gm.userId = ?', [userId]) as any;
  ...
  await db.none('UPDATE guilds SET treasury = treasury + ? WHERE id = ?', [tax, member.guildId]);
  await db.none('INSERT INTO guild_treasury_log (guildId, userId, amount, type) VALUES (?, ?, ?, ?)', [member.guildId, userId, tax, source]);
  return income - tax;
}
...
export async function applyExp(db: DB, userId: number, expGain: number, currentExp: number, currentLevel: number, currentStatPoints: number): {
