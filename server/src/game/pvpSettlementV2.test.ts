...
test('planned income is capped to locked loser balance across settlement and history', async () => {
  ...
  const basePlan = plan();
  const result = await settlePvpV2(client, plan({ history: {
    ...basePlan.history,
    steps: (actual: number) => [{ type: 'mercy', message: `actual:${actual}` }],
  } }));
  ...
  assert.equal(history.params?.[4], JSON.stringify([{ type: 'mercy', message: 'actual:5' }]));
