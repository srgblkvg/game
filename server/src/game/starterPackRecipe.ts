...
export function applyStarterPackRecipe(inventoryJson:string,templates:StarterEquipmentTemplate[],essenceCatalog:InventoryCatalogItem[],createId:()=>string|number):Result{
 ...
 const uuid=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;const existingIds=new Set<string>();for(const item of inventory){if(!item||typeof item!=='object'||(typeof item.id!=='string'&&typeof item.id!=='number'))return{ok:false,reason:'invalid-inventory'};const id=String(item.id);if((typeof item.id==='string'&&!uuid.test(item.id))||(typeof item.id==='number'&&(!Number.isFinite(item.id)||item.id<=0))||existingIds.has(id))return{ok:false,reason:'invalid-inventory'};if(item.type==='craft_item'||item.type==='material'||item.type==='upgrade'){if(!Number.isInteger(item.count)||item.count<0)return{ok:false,reason:'invalid-inventory'};}else if(typeof item.slot!=='string'||!item.slot)return{ok:false,reason:'invalid-inventory'};existingIds.add(id);}
