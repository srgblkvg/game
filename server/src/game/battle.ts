...
const dodgeChance = (defA: number, atkM: number, defExtraDodge: number) =>
  Math.max(0, (defA / (defA + 150)) * (1 - atkM / (atkM + 100)) + Math.min(0.5, defExtraDodge / 300));

const critChance = (m: number, extraCrit: number) => Math.min(0.8, m / (m + 150) + extraCrit / 300);
const critMult = (m: number) => 1.5 + 0.5 * (m / (m + 50));

const blockChance = (d: number, extraFullBlock: number) => Math.min(1, d / (d + 150) + extraFullBlock / 300);
