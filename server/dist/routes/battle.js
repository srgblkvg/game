...
router.post('/battle', (req, res) => {
    ...
    const attacker = database_1.default.prepare('SELECT * FROM users WHERE id = ?').get(userId);
    if (!attacker)
    ...
    let defender;
    if (opponentId) {
        defender = database_1.default.prepare('SELECT * FROM users WHERE id = ?').get(opponentId);
        if (!defender || defender.id == userId)
            return res.status(400).json({ error: 'Invalid opponent' });
    }
    else {
        const others = database_1.default.prepare('SELECT * FROM users WHERE id != ? AND (protectionUntil IS NULL OR protectionUntil < ?)').all(userId, now);
        if (others.length === 0)
            return res.status(400).json({ error: 'Все игроки защищены' });
    ...
    const moneyStolen = result.steps.find((s) => s.type === 'money')?.amount || 0;
    if (moneyStolen > 0) {
        if (result.winnerId === attacker.id) {
            database_1.default.prepare('UPDATE users SET money = money - ? WHERE id = ?').run(moneyStolen, defender.id);
            database_1.default.prepare('UPDATE users SET money = money + ? WHERE id = ?').run(moneyStolen, attacker.id);
        }
        else {
            database_1.default.prepare('UPDATE users SET money = money - ? WHERE id = ?').run(moneyStolen, attacker.id);
            database_1.default.prepare('UPDATE users SET money = money + ? WHERE id = ?').run(moneyStolen, defender.id);
    ...
    if (newLevel > 10)
        newLevel = 10;
    database_1.default.prepare(`UPDATE users SET level=?, exp=?, money=money+?, totalBattles=totalBattles+1, wins=wins+?, currentHp=?, lastAttackTime=?, lastHpUpdate=? WHERE id=?`)
        .run(newLevel, newExp, result.moneyGained, result.winnerId === attacker.id ? 1 : 0, result.attackerHpAfter, now, now, attacker.id);
    database_1.default.prepare(`UPDATE users SET currentHp=?, totalBattles=totalBattles+1, protectionUntil=?, lastHpUpdate=? WHERE id=?`)
        .run(result.defenderHpAfter, now + 3600, now, defender.id);
    database_1.default.prepare(`INSERT INTO battles (attackerId, defenderId, winnerId, log, steps, attackerHpAfter, defenderHpAfter, expGained, moneyGained, moneyStolen)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(attacker.id, defender.id, result.winnerId, JSON.stringify(result.log), JSON.stringify(result.steps), result.attackerHpAfter, result.defenderHpAfter, result.expGained, result.moneyGained, moneyStolen);
    const updatedAttacker = database_1.default.prepare('SELECT money FROM users WHERE id = ?').get(userId);
    res.json({
        log: result.log,
        steps: result.steps,
        ...
        opponent: {
            ...
            stats: (0, stats_1.currentStats)(defenderData.base, defenderData.equipment),
        },
...
router.get('/battles', (req, res) => {
    ...
    const battles = database_1.default.prepare(`
    SELECT b.*, 
