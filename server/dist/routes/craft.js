...
router.post('/craft/execute', (req, res) => {
    ...
    const recipe = database_1.default.prepare('SELECT * FROM craft_recipes WHERE id = ?').get(Number(recipe_id));
    if (!recipe)
        return res.status(400).json({ error: 'Рецепт не найден' });
    const ingredients = database_1.default.prepare(`
  ...
  `).all(recipe.id);
    ...
    for (const [itemId, needed] of ingredientMap) {
        const existing = inventory.find((i) => isCraftItem(i) && Number(i.id) === itemId);
    ...
    if (success) {
        if (recipe.result_type === 'item') {
            const resultItem = database_1.default.prepare('SELECT * FROM items WHERE id = ?').get(recipe.result_id);
            if (!resultItem)
            ...
            const resultCraftItem = database_1.default.prepare('SELECT * FROM craft_items WHERE id = ?').get(recipe.result_id);
            if (!resultCraftItem)
                return res.status(500).json({ error: 'Результирующий ресурс не найден' });
            const existing = newInventory.find((i) => isCraftItem(i) && Number(i.id) === recipe.result_id);
            if (existing) {
                existing.count += 1;
            }
            else {
        ...
        database_1.default.prepare('UPDATE users SET inventory = ?, money = ? WHERE id = ?').run(JSON.stringify(newInventory), newMoney, userId);
...
router.get('/craft/upgrade-info/:level', (req, res) => {
    ...
    const data = database_1.default.prepare('SELECT chance, money_cost FROM upgrade_chances WHERE level = ?').get(level);
    if (!data)
...
router.post('/craft/upgrade', (req, res) => {
    ...
    const upgradeData = database_1.default.prepare('SELECT chance, money_cost FROM upgrade_chances WHERE level = ?').get(targetLevel);
    if (!upgradeData) {
    ...
    if (success) {
        ...
        if (itemIdx !== -1) {
            ...
            const craftItem = database_1.default.prepare('SELECT id, name, rarity, type FROM craft_items WHERE rarity = ? AND type = \'craft\'').get(rarity);
            if (craftItem) {
