...
const dodgeChance = (defA, atkM, defExtraDodge) => Math.max(0, (defA / (defA + 50)) * (1 - atkM / (atkM + 100)) + Math.min(1, defExtraDodge / 100));
const critChance = (m, extraCrit) => Math.min(1, m / (m + 50) + extraCrit / 100);
const critMult = (m) => 1.5 + 0.5 * (m / (m + 50));
const blockChance = (d, extraFullBlock) => Math.min(1, d / (d + 50) + extraFullBlock / 100);
const blockRed = (d) => Math.min(d, 75) / 100;
const counterChance = (defStats, atkStats, defExtraCounter) => {
...
const stunChance = (atkStats, defStats) => {
...
function runBattle(attacker, defender) {
    const statsA = (0, stats_1.currentStats)(attacker.base, attacker.equipment);
    const statsD = (0, stats_1.currentStats)(defender.base, defender.equipment);
    let hpA = statsA.hp;
    let hpD = statsD.hp;
    ...
    let turn = statsA.a >= statsD.a ? 'A' : 'D';
    ...
    while (hpA > 1 && hpD > 1 && turns < maxTurns) {
        ...
        if (turn === 'A') {
            ...
            if (!stunnedA)
                stamA = Math.min(statsA.maxStamina, stamA + statsA.regen);
            else {
            ...
            // Ход защитника (бот)
            if (!stunnedD)
                stamD = Math.min(statsD.maxStamina, stamD + statsD.regen);
            else {
