...
async function go() {
  ...
  let r;
  r = await fetch(B + '/bots/start', { method: 'POST', headers: { ...h, 'Content-Type': 'application/json' }, body: JSON.stringify({ count: 5, useExisting: true }) });
  console.log('START:', JSON.stringify(await r.json()));
  ...
  r = await fetch(B + '/bots/stop', { method: 'POST', headers: h });
  console.log('STOP:', JSON.stringify(await r.json()));
}
