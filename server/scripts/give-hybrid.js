"use strict";
// @ts-nocheck
// Гибрид: Буревестник 4 + Крушитель 2 + артефакты (blockPen 25% + dodge)
// Слот 3, curse S 2-4
...
async function main() {
    ...
    set3.boots = { id: now(), name: 'Сапоги урагана', slot: 'boots', rarity_id: 6, rarity_display: 'Мифический', rarity_color: '#e74c3c',
        ...
        extra: { set: 'Буревестник', crit: 0, dodge: 0, counter: 0, fullBlock: 0, setBonus2: '+10% ловкость', setBonus3: 'Первый ход всегда твой', setBonus4: '+20% уклонение' },
        ...curseS(randomRank()) };
    // Крушитель 2 (weapon1 + shield → blockPen 25%)
    set3.weapon1 = { id: now(), name: 'Молот крушителя', slot: 'weapon1', rarity_id: 6, rarity_display: 'Мифический', rarity_color: '#e74c3c',
        bonuses: { s: 28, a: 0, d: 20, m: 0 }, image: 'sword/sword_gray.webp', upgradeLevel: 6,
        extra: { set: 'Крушитель', crit: 0, dodge: 0, counter: 0, fullBlock: 0, setBonus2: '+25% пробивание блока', setBonus3: '+10% крит', setBonus4: '+15% урон' },
        ...curseS(randomRank()) };
    set3.shield = { id: now(), name: 'Дробящий щит', slot: 'shield', rarity_id: 6, rarity_display: 'Мифический', rarity_color: '#e74c3c',
        bonuses: { s: 0, a: 0, d: 0, m: 22 }, image: 'shield/shield_gray.webp', upgradeLevel: 6,
        extra: { set: 'Крушитель', crit: 0, dodge: 0, counter: 0, fullBlock: 22, setBonus2: '+25% пробивание блока', setBonus3: '+10% крит', setBonus4: '+15% урон' },
        ...curseS(randomRank()) };
    // Остальное
    ...
    set3.ring2 = { id: now(), name: 'Печатка разорванных душ', slot: 'ring', rarity_id: 6, rarity_display: 'Мифический', rarity_color: '#e74c3c',
        ...
        ...curseS(randomRank()) };
    await index_1.db.run('UPDATE users SET equipment_3 = ?::jsonb WHERE id = ?', [JSON.stringify(set3), userId]);
    console.log('Слот III (Гибрид dodge+blockPen):');
    for (const key of Object.keys(set3)) {
        const item = set3[key];
        console.log(`  + ${item.name.padEnd(24)} [${item.slot}] curse ${item.curseName} +${item.curseValue} S`);
    }
    console.log('\nГотово.');
    process.exit(0);
}
main().catch(e => { console.error(e); process.exit(1); });
//# sourceMappingURL=give-hybrid.js.map