...
async function cleanupGuests() {
    ...
    const countRow = await index_1.db.one('SELECT COUNT(*) as cnt FROM users WHERE isGuest = 1 AND lastaction < $1', [cutoff]);
    ...
    // Удаляем логи входа
    await index_1.db.run(`DELETE FROM login_logs WHERE userId IN (
      SELECT id FROM users WHERE isGuest = 1 AND lastaction < $1
    )`, [cutoff]);
    // Удаляем гостей
    const result = await index_1.db.run('DELETE FROM users WHERE isGuest = 1 AND lastaction < $1', [cutoff]);
...
//# sourceMappingURL=cleanup-guests.js.map