...
router.post('/actions', async (req, res) => {
    ...
    await index_1.db.run('INSERT INTO actions_config (section, title, subtitle, icon, bg_image, path, cost, sort_order) VALUES (?,?,?,?,?,?,?,?)', [section || 'world', title, subtitle || '', icon || '', bg_image || null, path || null, cost || 0, sort_order || 0]);
    res.json({ success: true });
});
router.put('/actions/:id', async (req, res) => {
    const { section, title, subtitle, icon, bg_image, path, cost, sort_order } = req.body;
    await index_1.db.run('UPDATE actions_config SET section=?,title=?,subtitle=?,icon=?,bg_image=?,path=?,cost=?,sort_order=? WHERE id=?', [section, title, subtitle, icon, bg_image || null, path || null, cost, sort_order, req.params.id]);
    res.json({ success: true });
});
router.delete('/actions/:id', async (req, res) => {
    await index_1.db.run('DELETE FROM actions_config WHERE id=?', [req.params.id]);
    res.json({ success: true });
});
// ---------- Мобы ----------
router.get('/mobs', async (req, res) => {
    const mobs = await index_1.db.query('SELECT * FROM mobs ORDER BY location, level', []);
    res.json(mobs);
});
router.put('/mobs/:id', async (req, res) => {
    const { name, level, hp, atk, agi, def, mst, xp, gold_min, gold_max, location, background, description, loot_junk, loot_common, loot_uncommon, loot_rare, loot_epic, loot_legendary, loot_mythic } = req.body;
    await index_1.db.run(`UPDATE mobs SET name=?,level=?,hp=?,atk=?,agi=?,def=?,mst=?,xp=?,gold_min=?,gold_max=?,
...
router.get('/mob-locations', async (req, res) => {
    const locs = await index_1.db.query('SELECT DISTINCT location FROM mobs ORDER BY location', []);
...
router.post('/floors', async (req, res) => {
    ...
    await index_1.db.run('INSERT INTO floors (name, background, sort_order) VALUES (?,?,?)', [name, background || null, sort_order || 0]);
    res.json({ success: true });
});
router.put('/floors/:id', async (req, res) => {
    const { name, background, sort_order } = req.body;
    await index_1.db.run('UPDATE floors SET name=?, background=?, sort_order=? WHERE id=?', [name, background || null, sort_order || 0, req.params.id]);
    res.json({ success: true });
});
router.delete('/floors/:id', async (req, res) => {
    await index_1.db.run('DELETE FROM floors WHERE id=?', [req.params.id]);
    res.json({ success: true });
});
// ---------- Сезоны ----------
router.get('/seasons', async (req, res) => {
    const seasons = await index_1.db.query('SELECT * FROM seasons ORDER BY id DESC LIMIT 10', []);
    res.json(seasons);
});
router.post('/seasons/finish', async (req, res) => {
    const didReset = await (0, rating_1.checkSeasonReset)();
    if (didReset) {
        const active = await index_1.db.one("SELECT * FROM seasons WHERE status = 'active' LIMIT 1");
