...
// ═══════ ТАБЛИЦЫ ═══════
index_1.db.run(`CREATE TABLE IF NOT EXISTS dungeon_runs (
...
index_1.db.run(`CREATE TABLE IF NOT EXISTS skill_pages (
    id SERIAL PRIMARY KEY,
...
index_1.db.run(`CREATE TABLE IF NOT EXISTS skill_levels (
    userId INTEGER NOT NULL,
    skillId INTEGER NOT NULL,
...
// ═══════ КОНСТАНТЫ ═══════
const WEAPON_SPEED = { 0: 0.3, 1: 0.5, 2: 0.7, 3: 0.9, 4: 1.1, 5: 1.3, 6: 1.5 };
const TICK_MS = 100;
const DAILY_RUNS_MAX = 4;
const BASE_SKILL_IDS = new Set([7, 2, 3]); // Рывок, Размах, Боевой клич
function getHpRegenRate(user) {
...
function skillBonus(level, perLevel) {
    return (level - 1) * perLevel;
}
const activeRuns = new Map();
function getAttackSpeed(rarity, weaponStrBonus) {
...
function calcEnemyDamage(enemy, floor) {
...
async function loadMobs() {
    if (!_mobCache)
        _mobCache = await index_1.db.query('SELECT name, background, level, hp, atk FROM mobs');
    return _mobCache;
}
function generateEnemyFromMob(mob, floor, isBoss) {
...
async function generateFloorEnemies(floor) {
...
function getAvailableSkills(userId, equippedSkills) {
    return (async () => {
        const levels = await index_1.db.query('SELECT skillId, level FROM skill_levels WHERE userId = ?', [userId]);
        const levelMap = {};
        for (const r of levels)
            levelMap[r.skillid] = r.level;
...
router.get('/dungeon/status', async (req, res) => {
    ...
    const lastRun = await index_1.db.one('SELECT dailyRuns, dailyRunDate, startedAt FROM dungeon_runs WHERE userId = ?', [userId]).catch(() => null);
    ...
    const checkpointRow = await index_1.db.one('SELECT checkpointFloor FROM dungeon_runs WHERE userId = ?', [userId]).catch(() => null);
...
router.post('/dungeon/start', async (req, res) => {
    ...
    // Данные игрока
    const user = await index_1.db.one('SELECT id, level, baseS, baseA, baseD, baseM, inventory, equipment, equipment_1, equipment_2, equipment_3, active_equip_slot, drinkuntil, activedrink, roomtype, roomuntil, premiumuntil FROM users WHERE id = ?', [userId]);
    if (!user)
    ...
    if (!equip || Object.keys(equip).length === 0)
        equip = parseEq(user.equipment_1);
    if (!equip || Object.keys(equip).length === 0)
        equip = parseEq(user.equipment);
    const weapon = equip.weapon1;
    const weaponRarity = weapon?.rarity_id ?? 0;
    // Получаем реальные статы персонажа
    const stats = await (0, helpers_1.buildPlayerStats)(user, 'pve');
    const playerMaxHp = stats.hp;
    const playerHp = stats.hp; // всегда с полным HP в бой
    // Чекпоинт
    let checkpointRow = await index_1.db.one('SELECT checkpointFloor FROM dungeon_runs WHERE userId = ?', [userId]).catch(() => null);
    const checkpoint = checkpointRow?.checkpointfloor || 0;
    ...
    try {
        await index_1.db.run(`INSERT INTO dungeon_logs (userId, floor, playerHp, playerMaxHp, playerStr, playerAgi, playerDef, playerMag, enemies, startedAt)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?::jsonb, ?)`, [userId, startFloor, playerHp, playerMaxHp, stats.s, stats.a, stats.d, stats.m,
            JSON.stringify(enemies.map(e => ({ name: e.name, hp: e.maxHp, dmg: e.dmg, interval: e._attackInterval }))),
            Math.floor(Date.now() / 1000)]);
    }
    catch (e) {
        console.error('[dungeon_logs]', e.message);
    }
    ...
    // Сохраняем в БД
    await index_1.db.run(`INSERT INTO dungeon_runs (userId, currentFloor, checkpointFloor, enemyData, playerHp, playerMaxHp, role, skills, startedAt, dailyRuns, dailyRunDate)
         VALUES (?, ?, ?, '[]', ?, ?, 'warrior', ?, ?, ?, ?)
         ON CONFLICT (userId) DO UPDATE SET
         currentFloor=?, checkpointFloor=?, enemyData='[]', playerHp=?, playerMaxHp=?, skills=?, startedAt=?, dailyRuns=?, dailyRunDate=?`, [userId, startFloor, checkpoint, playerMaxHp, playerMaxHp, JSON.stringify(equippedSkillIds || []), run.startedAt, run.dailyRuns, today,
...
router.post('/dungeon/skill', async (req, res) => {
    ...
    const run = activeRuns.get(userId);
    if (!run)
        return res.status(400).json({ error: 'Данж не активен' });
    if (run.playerHp <= 0)
        return res.status(400).json({ error: 'Вы мертвы' });
    const skill = run.skills.find(s => s.id === skillId);
    if (!skill)
        return res.status(400).json({ error: 'Скилл не найден' });
    const now = Date.now() / 1000;
    // Кулдаун
...
router.post('/dungeon/target', async (req, res) => {
    ...
    if (idx === -1)
        return res.status(400).json({ error: 'Враг не найден' });
...
router.post('/dungeon/claim', async (req, res) => {
    ...
    if (run.enemies.length > 0)
        return res.status(400).json({ error: 'Сначала убейте всех врагов' });
    // Награда за этаж
    const floor = run.currentFloor;
    const silverReward = 10 + floor * 15 + Math.floor(Math.random() * floor * 10);
    await index_1.db.run('UPDATE users SET money = money + ? WHERE id = ?', [silverReward, userId]);
    // Шанс руны/предмета
    let itemReward = null;
    ...
    if (Math.random() < dropChance) {
        ...
        if (rarityRoll < 0.5)
            ...
            rarityId = 4;
        const craftItem = await index_1.db.one('SELECT c.id, c.name, c.rarity_id, c.type, c.image, r.display_name, r.color FROM craft_items c JOIN rarities r ON c.rarity_id = r.id WHERE c.rarity_id = ? AND c.type = \'upgrade\' ORDER BY RANDOM() LIMIT 1', [rarityId]).catch(() => null);
        if (craftItem) {
            itemReward = { name: craftItem.name, rarity: craftItem.display_name, image: craftItem.image };
            const inventory = JSON.parse((await index_1.db.one('SELECT inventory FROM users WHERE id = ?', [userId])).inventory || '[]');
    ...
    if (Math.random() < pageChance) {
        ...
        if (!randomSkill)
            return;
        await index_1.db.run('INSERT INTO skill_pages (userId, skillId, count) VALUES (?, ?, 1) ON CONFLICT (userId, skillId) DO UPDATE SET count = skill_pages.count + 1', [userId, randomSkill.id]);
    ...
    if (isBossFloor && floor > run.checkpointFloor) {
        ...
        await index_1.db.run('UPDATE dungeon_runs SET checkpointFloor = ? WHERE userId = ?', [floor + 1, userId]);
    }
    // НЕ останавливаем тик — игрок остаётся в комнате, регенит HP
    // Сохраняем состояние для продолжения/выхода
    await index_1.db.run(`UPDATE dungeon_runs SET currentFloor = ?, playerHp = ?, playerMaxHp = ?, checkpointFloor = ? WHERE userId = ?`, [floor + 1, run.playerHp, run.playerMaxHp, run.checkpointFloor, userId]);
    res.json({
        success: true,
        floor: floor,
        nextFloor: floor + 1,
...
router.post('/dungeon/continue', async (req, res) => {
    ...
    const saved = await index_1.db.one('SELECT currentFloor, playerHp, playerMaxHp, checkpointFloor FROM dungeon_runs WHERE userId = ?', [userId]).catch(() => null);
    if (!saved)
        return res.status(400).json({ error: 'Нет сохранённого захода' });
    const user = await index_1.db.one('SELECT id, level, baseS, baseA, baseD, baseM, inventory, equipment, equipment_1, equipment_2, equipment_3, active_equip_slot, drinkuntil, activedrink, roomtype, roomuntil, premiumuntil FROM users WHERE id = ?', [userId]);
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    const activeSlot = user.active_equip_slot || 1;
    ...
    if (existingRun?.tickTimer)
        clearInterval(existingRun.tickTimer); // остановим старый тик
    const run = {
...
router.post('/dungeon/flee', async (req, res) => {
    ...
    await index_1.db.run('DELETE FROM dungeon_runs WHERE userId = ?', [userId]);
...
router.get('/dungeon/pages', async (req, res) => {
    const userId = req.userId;
    const pages = await index_1.db.query('SELECT skillId, count FROM skill_pages WHERE userId = ? ORDER BY skillId', [userId]);
    const levels = await index_1.db.query('SELECT skillId, level FROM skill_levels WHERE userId = ?', [userId]);
    const countMap = {};
    for (const p of pages)
        countMap[p.skillid] = p.count;
    const levelMap = {};
    for (const r of levels)
        levelMap[r.skillid] = r.level;
...
router.post('/dungeon/upgrade-skill', async (req, res) => {
    const userId = req.userId;
    const { skillId } = req.body;
    const curLevel = await index_1.db.one('SELECT level FROM skill_levels WHERE userId = ? AND skillId = ?', [userId, skillId]).catch(() => null);
    const level = curLevel?.level || 0;
    const neededPages = 10 + level * 15;
    const neededSilver = 1000 * Math.pow(3, level);
    const pages = await index_1.db.one('SELECT count FROM skill_pages WHERE userId = ? AND skillId = ?', [userId, skillId]).catch(() => null);
    ...
    await index_1.db.run('UPDATE users SET money = money - ? WHERE id = ?', [neededSilver, userId]);
    await index_1.db.run('UPDATE skill_pages SET count = count - ? WHERE userId = ? AND skillId = ?', [neededPages, userId, skillId]);
    await index_1.db.run('INSERT INTO skill_levels (userId, skillId, level) VALUES (?, ?, 1) ON CONFLICT (userId, skillId) DO UPDATE SET level = skill_levels.level + 1', [userId, skillId]);
    const updated = await index_1.db.one('SELECT level FROM skill_levels WHERE userId = ? AND skillId = ?', [userId, skillId]);
...
function tickCombat(run) {
    ...
    if (run.playerHp <= 0) {
        ...
        if (run.tickTimer)
            clearInterval(run.tickTimer);
        index_1.db.run('DELETE FROM dungeon_runs WHERE userId = ?', [run.userId]);
    }
}
