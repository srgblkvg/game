...
router.get('/tutorial/state', async (req, res) => {
    ...
    const user = await index_1.db.one('SELECT tutorial_step, tutorial_completed FROM users WHERE id = ?', [userId]);
    if (!user)
...
router.post('/tutorial/pve', async (req, res) => {
    ...
    if ((user.tutorial_step || 0) !== 0)
        return res.status(400).json({ error: 'Неверный шаг обучения' });
    // First mob: Костяная крыса (id=1, level=1)
    const mob = await index_1.db.one('SELECT * FROM mobs WHERE id = 1');
    if (!mob)
        return res.status(500).json({ error: 'Моб не найден' });
    ...
    // Add Пыль забвения
    const existingDust = inventory.find((i) => i.type === 'craft_item' && i.id === 1);
    if (existingDust) {
    ...
    await index_1.db.run('UPDATE users SET inventory = ?, money = ?, tutorial_step = 1, lastpveattacktime = ? WHERE id = ?', [JSON.stringify(inventory), newMoney, Math.floor(Date.now() / 1000), userId]);
    res.json({
        success: true,
        steps,
        loot: {
...
router.post('/tutorial/equip', async (req, res) => {
    ...
    await index_1.db.run('UPDATE users SET tutorial_step = 2 WHERE id = ?', [userId]);
...
router.post('/tutorial/craft', async (req, res) => {
    ...
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    if ((user.tutorial_step || 0) !== 2)
        return res.status(400).json({ error: 'Неверный шаг обучения' });
    const inventory = JSON.parse(user.inventory || '[]');
    // Find Пыль забвения
    const dustIdx = inventory.findIndex((i) => i.type === 'craft_item' && i.id === 1);
    if (dustIdx === -1) {
    ...
    await index_1.db.run('UPDATE users SET inventory = ?, tutorial_step = 3 WHERE id = ?', [JSON.stringify(inventory), userId]);
    res.json({
        success: true,
        crafted: {
            name: 'Гробовая преграда',
...
router.post('/tutorial/equip-shield', async (req, res) => {
    ...
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    if ((user.tutorial_step || 0) !== 3)
        return res.status(400).json({ error: 'Неверный шаг обучения' });
    const equipment = JSON.parse(user.equipment || '{}');
    if (!equipment.shield) {
        return res.status(400).json({ error: 'Сначала наденьте щит' });
    }
    await index_1.db.run('UPDATE users SET tutorial_step = 4 WHERE id = ?', [userId]);
...
router.post('/tutorial/arena', async (req, res) => {
    ...
    await index_1.db.run('UPDATE users SET tutorial_step = 5, lastpvptime = ? WHERE id = ?', [Math.floor(Date.now() / 1000), userId]);
...
router.post('/tutorial/complete', async (req, res) => {
    ...
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    if ((user.tutorial_step || 0) !== 5)
        return res.status(400).json({ error: 'Неверный шаг обучения' });
    const reward = 1000;
    await index_1.db.run('UPDATE users SET tutorial_step = 6, tutorial_completed = 1, money = money + ? WHERE id = ?', [reward, userId]);
