...
router.post('/character/equip', async (req, res) => {
    ...
    const collectionCount = await (0, helpers_1.getCollectionBonus)(userId);
    ...
    if (itemId === undefined || itemId === null) {
        ...
        const oldStats = (0, stats_1.currentStats)(base, equipment, drinkBonuses, collectionCount, guildBonus);
        const oldMaxHp = oldStats.hp;
        inventory.push(currentEquipped);
        delete equipment[slotId];
        const newStats = (0, stats_1.currentStats)(base, equipment, drinkBonuses, collectionCount, guildBonus);
        const newMaxHp = newStats.hp;
        const newHp = (0, helpers_1.recalcHpOnEquip)(user.currentHp, oldMaxHp, newMaxHp);
        const now = Math.floor(Date.now() / 1000);
        const activeSlot = user.active_equip_slot || 1;
        await index_1.db.run('UPDATE users SET inventory = ?, equipment = ?, currentHp = ?, lastHpUpdate = ? WHERE id = ?', [JSON.stringify(inventory), JSON.stringify(equipment), newHp, now, userId]);
        // Синхронизируем с активным equipment_N
        await index_1.db.run(`UPDATE users SET equipment_${activeSlot} = ?::jsonb WHERE id = ?`, [JSON.stringify(equipment), userId]);
    ...
    if (!item || item.type === 'material' || item.type === 'craft_item')
        return res.status(400).json({ error: 'Нельзя надеть материал или ресурс' });
    ...
    const newStats = (0, stats_1.currentStats)(base, equipment, drinkBonuses, collectionCount, guildBonus);
    const newMaxHp = newStats.hp;
    const newHp = (0, helpers_1.recalcHpOnEquip)(user.currentHp, oldStats.hp, newMaxHp);
    const now = Math.floor(Date.now() / 1000);
    const activeSlot = user.active_equip_slot || 1;
    await index_1.db.run('UPDATE users SET inventory = ?, equipment = ?, currentHp = ?, lastHpUpdate = ? WHERE id = ?', [JSON.stringify(inventory), JSON.stringify(equipment), newHp, now, userId]);
    // Синхронизируем с активным equipment_N
    ...
    inventory = inventory.filter((item) => {
        ...
        if (idsToDelete.has(itemIdStr) && item.type !== 'craft_item') {
            ...
            if (existing)
                ...
                materialsToAdd.push({ rarity_id: rarityId, count: 1 });
            return false;
        }
        return true;
    });
...
router.post('/character/expand-inventory', async (req, res) => {
    ...
    if (currentSlots >= MAX_SLOTS)
        return res.status(400).json({ error: `Достигнут максимум слотов (${MAX_SLOTS})` });
    ...
    await index_1.db.run('UPDATE users SET money = money - ?, inventorySlots = inventorySlots + 1 WHERE id = ?', [price, userId]);
...
router.post('/character/reorder-inventory', async (req, res) => {
    ...
    await index_1.db.run('UPDATE users SET inventory = ? WHERE id = ?', [JSON.stringify(reordered), userId]);
