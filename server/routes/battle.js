...
router.post('/battle', async (req, res) => {
    const parsed = validation_1.battleSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: 'Некорректные данные боя' });
    const userId = req.userId;
    const { opponentId } = parsed.data;
    const now = Math.floor(Date.now() / 1000);
    const attacker = await index_1.db.one(`SELECT ${helpers_1.USER_BATTLE_FIELDS_GUILD} FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id = ?`, [userId]);
    if (!attacker)
    ...
    if (attacker.faction === 'bandit')
        attackCooldown = Math.floor(attackCooldown / 2);
    ...
    let defender;
    if (opponentId) {
        defender = await index_1.db.one(`SELECT ${helpers_1.USER_BATTLE_FIELDS_GUILD} FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id = ?`, [opponentId]);
        if (!defender || defender.id == userId)
            return res.status(400).json({ error: 'Invalid opponent' });
    }
    else {
        const range = attacker.faction === 'bandit' ? 4 : 2;
        const others = await index_1.db.query(`SELECT ${helpers_1.USER_BATTLE_FIELDS_GUILD} FROM users u LEFT JOIN guilds g ON u.guildId = g.id WHERE u.id != ? AND u.id > 0 AND (u.protectionUntil IS NULL OR u.protectionUntil < ?) AND u.level >= ? AND u.level <= ?`, [userId, now, attacker.level - range, attacker.level + range]);
        if (others.length === 0)
            return res.status(400).json({ error: 'Все игроки защищены или вне диапазона уровней' });
    ...
    const aGuildBonus = await (0, guildBuildings_1.getGuildBonus)(attacker.id, 'arena');
    const attackerStats = await (0, helpers_1.buildPlayerStats)(attacker, 'arena');
    const attackerMaxHp = attackerStats.hp;
    const attackerCurrentHp = await (0, hpRegen_1.applyHpRegen)({
        id: attacker.id,
    ...
    const dGuildBonus = await (0, guildBuildings_1.getGuildBonus)(defender.id, 'arena');
    const defenderStats = await (0, helpers_1.buildPlayerStats)(defender, 'arena');
    const defenderMaxHp = defenderStats.hp;
    const defenderCurrentHp = await (0, hpRegen_1.applyHpRegen)({
        id: defender.id,
    ...
    const attackerData = {
        ...
        collectionBonus: await (0, helpers_1.getCollectionBonus)(attacker.id),
    ...
    const attackerFullStats = (0, stats_1.currentStats)(attackerData.base, attackerData.equipment, attackerData.drinkBonuses, attackerData.collectionBonus, attackerData.guildBonus);
    ...
    if (mercyThreshold) {
        ...
        const newAttackerElo = (0, rating_1.calcElo)(attacker.elo || 1000, defender.elo || 1000, true, attacker.level);
        const newDefenderElo = (0, rating_1.calcElo)(defender.elo || 1000, attacker.elo || 1000, false, defender.level);
        const eloChange = newAttackerElo - (attacker.elo || 1000);
        const attExp = await (0, helpers_1.applyExp)(attacker.id, expGained, attacker.exp, attacker.level, attacker.statPoints || 0);
        // Атакующий
        const attackerMoneyAfterTax = moneyStolen > 0
            ? await (0, helpers_1.collectGuildTax)(attacker.id, moneyStolen, 'tax_pvp')
            : 0;
        await index_1.db.run(`UPDATE users SET level=?, exp=?, money=money+?, totalBattles=totalBattles+1, wins=wins+1, lastAttackTime=?, lastHpUpdate=?, statPoints=statPoints+?, elo=?, seasonWins=seasonWins+1, lastPvpTime=?, totalPvpMoneyWon=totalPvpMoneyWon+?, arenaOpponentId=NULL WHERE id=?`, [attExp.newLevel, attExp.newExp, attackerMoneyAfterTax, now, now, attExp.levelsGained * 5, Math.max(100, newAttackerElo), now, moneyStolen, attacker.id]);
        // --- Обновление защитника ---
        const protUntil = now + 3600;
        const actualStolen = Math.min(moneyStolen, defender.money);
        await index_1.db.run(`UPDATE users SET money=money-?, totalBattles=totalBattles+1, protectionUntil=?, elo=?, seasonLosses=seasonLosses+1, lastPvpTime=?, totalPvpMoneyLost=totalPvpMoneyLost+? WHERE id=?`, [actualStolen, protUntil, Math.max(100, newDefenderElo), now, actualStolen, defender.id]);
        (0, events_1.sendToUser)(defender.id, { type: 'protection', protectionUntil: protUntil });
        ...
        if (attackerData.faction === 'guard') {
            ...
            await index_1.db.run('UPDATE users SET karma = GREATEST(-100, LEAST(100, karma + ?)) WHERE id = ?', [karmaChange, attacker.id]);
        }
        // Репутация бандита: +1 за победу в PvP
        if (attackerData.faction === 'bandit') {
            await index_1.db.run('UPDATE users SET bandit_reputation = bandit_reputation + 1 WHERE id = ?', [attacker.id]);
        }
        // Запись в историю
        await index_1.db.run(`INSERT INTO battles (attackerId, defenderId, winnerId, log, steps, attackerHpAfter, defenderHpAfter, expGained, moneyGained, moneyStolen)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [attacker.id, defender.id, attacker.id, JSON.stringify([`${attacker.username} подавляет ${defender.username} без боя`]),
            JSON.stringify([
        ...
        const updatedAttacker = await index_1.db.one('SELECT money FROM users WHERE id = ?', [userId]);
        // Guild quest progress — track PvP win (mercy)
        if (attacker.guildId) {
            (0, guild_1.updateGuildQuestProgress)(attacker.guildId, 'pvp').catch(e => console.error('guildQuest PvP mercy:', e.message));
        }
        // Достижения и квесты
        (0, achievements_1.checkAchievement)(attacker.id, 'pvp_wins').catch(() => { });
        if (moneyStolen > 0)
            (0, achievements_1.trackIncome)(attacker.id, moneyStolen).catch(() => { });
        (0, events_1.markDirty)(attacker.id, 'quests');
        return res.json({
            mercy: true,
    ...
    let moneyStolen = result.steps.find((s) => s.type === 'money')?.amount || 0;
    // Бандит: +1% к награбленному за каждые 100 репутации
    const attackerReputation = (attacker.bandit_reputation || 0);
    if (attackerData.faction === 'bandit' && attackerReputation > 0) {
        moneyStolen = Math.floor(moneyStolen * (1 + Math.floor(attackerReputation / 100) / 100));
    ...
    const newAttackerElo = (0, rating_1.calcElo)(attacker.elo || 1000, defender.elo || 1000, attackerWon, attacker.level);
    const newDefenderElo = (0, rating_1.calcElo)(defender.elo || 1000, attacker.elo || 1000, !attackerWon, defender.level);
    // --- Обновление атакующего ---
    const attExp = await (0, helpers_1.applyExp)(attacker.id, result.winnerId === attacker.id ? result.expGained : 0, attacker.exp, attacker.level, attacker.statPoints || 0);
    ...
    if (attExp.levelsGained > 0 && attacker.oauthProvider === 'vk' && attacker.oauthId) {
        (0, vkLeaderboard_1.sendLeaderboardLevel)(attacker.id, attExp.newLevel, String(attacker.oauthId)).catch(() => { });
    }
    ...
    // --- Обновление защитника ---
    const defExp = await (0, helpers_1.applyExp)(defender.id, result.winnerId === defender.id ? result.expGained : 0, defender.exp, defender.level, defender.statPoints || 0);
    ...
    const defenderActualDelta = defenderMoneyDelta < 0
        ...
        : defenderMoneyDelta;
    await index_1.db.run(`UPDATE users SET level=?, exp=?, money=money+?, totalBattles=totalBattles+1, wins=wins+?, currentHp=?, protectionUntil=?, lastHpUpdate=?, statPoints = statPoints + ?, elo=?, seasonWins=seasonWins+?, seasonLosses=seasonLosses+?, lastPvpTime=?, totalPvpMoneyWon=totalPvpMoneyWon+?, totalPvpMoneyLost=totalPvpMoneyLost+? WHERE id=?`, [defExp.newLevel, defExp.newExp, defenderActualDelta, !attackerWins ? 1 : 0, result.defenderHpAfter, now + 3600, now, defExp.levelsGained * 5, Math.max(100, newDefenderElo), attackerWon ? 0 : 1, attackerWon ? 1 : 0, now,
        !attackerWins ? moneyStolen : 0, !attackerWins ? 0 : -defenderActualDelta, defender.id]);
    (0, events_1.sendToUser)(defender.id, { type: 'protection', protectionUntil: now + 3600 });
    // Достижения — защитник
    if (!attackerWins) {
        (0, achievements_1.checkAchievement)(defender.id, 'pvp_wins').catch(() => { });
        if (moneyStolen > 0)
            (0, achievements_1.trackIncome)(defender.id, moneyStolen).catch(() => { });
    }
    // VK Leaderboard — защитник
    if (defExp.levelsGained > 0 && defender.oauthProvider === 'vk' && defender.oauthId) {
        (0, vkLeaderboard_1.sendLeaderboardLevel)(defender.id, defExp.newLevel, String(defender.oauthId)).catch(() => { });
    }
    // Guild quest progress — track PvP wins
    const w = await index_1.db.one('SELECT guildId FROM users WHERE id = ?', [result.winnerId]);
    if (w?.guildId) {
        (0, guild_1.updateGuildQuestProgress)(w.guildId, 'pvp').catch(e => console.error('guildQuest PvP:', e.message));
    }
    // Карма Стражника: +1 за бандита, -1 за остальных
    ...
    if (winnerFaction === 'guard') {
        ...
        await index_1.db.run('UPDATE users SET karma = GREATEST(-100, LEAST(100, karma + ?)) WHERE id = ?', [karmaChange, result.winnerId]);
    }
    // Репутация бандита: +1 за победу в PvP
    if (winnerFaction === 'bandit') {
        await index_1.db.run('UPDATE users SET bandit_reputation = bandit_reputation + 1 WHERE id = ?', [result.winnerId]);
    }
    (0, events_1.markDirty)(result.winnerId, 'quests');
    ...
    if (!attacker.tutorialCompleted && (attacker.tutorialStep || 0) < 4) {
        ...
        if (newStep >= 4) {
            await index_1.db.run('UPDATE users SET tutorial_step = 4, tutorial_completed = 1 WHERE id = ?', [attacker.id]);
        }
        else {
            await index_1.db.run('UPDATE users SET tutorial_step = ? WHERE id = ?', [newStep, attacker.id]);
        }
    }
...
router.get('/battles', async (req, res) => {
    ...
    const battles = await index_1.db.query(`
    SELECT b.*, 
...
exports.adminRouter.get('/battles', async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
    const total = (await index_1.db.one('SELECT COUNT(*) as cnt FROM battles', [])).cnt;
    const battles = await index_1.db.query(`
