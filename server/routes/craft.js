...
router.get('/craft/recipes', async (req, res) => {
    const recipes = await index_1.db.query('SELECT * FROM craft_recipes ORDER BY id', []);
    for (const recipe of recipes) {
        recipe.ingredients = await index_1.db.query(`
    ...
    `, [recipe.id]);
        if (recipe.result_type === 'item') {
            const rawResult = await index_1.db.one(`
      ...
      `, [recipe.result_id]);
            ...
            recipe.result = await index_1.db.one('SELECT id as rarity_id, display_name as rarity_display, color as rarity_color, name FROM rarities WHERE id = ?', [recipe.result_id]) || null;
            if (recipe.result)
                recipe.result.name = `Случайный предмет (${recipe.result.rarity_display})`;
        }
        else if (recipe.result_type === 'craft_item') {
            recipe.result = await index_1.db.one(`
      ...
      `, [recipe.result_id]) || null;
        ...
        // Категория
        recipe.category = await index_1.db.one('SELECT * FROM craft_recipe_categories WHERE id = ?', [recipe.category_id]) || null;
    }
    res.json(recipes);
});
...
router.post('/craft/execute', async (req, res) => {
    ...
    const recipe = await index_1.db.one('SELECT * FROM craft_recipes WHERE id = ?', [Number(recipe_id)]);
    if (!recipe)
        return res.status(400).json({ error: 'Рецепт не найден' });
    const ingredients = await index_1.db.query(`
  ...
  `, [recipe.id]);
    ...
    for (const [itemId, needed] of ingredientMap) {
        const existing = inventory.find((i) => isCraftItem(i) && String(i.id) === String(itemId));
    ...
    if (success) {
        ...
        if (recipe.result_type === 'item') {
            const resultItem = await index_1.db.one(`
      ...
      `, [recipe.result_id]);
            if (!resultItem)
                return res.status(500).json({ error: 'Результирующий предмет не найден' });
            craftedItem = {
            ...
            const randomItem = await index_1.db.one(`
                SELECT i.*, r.display_name as rarity_display, r.color as rarity_color
                FROM items i
                JOIN rarities r ON i.rarity_id = r.id
                WHERE i.rarity_id = ?
            ...
            `, [rarityId]);
            if (!randomItem)
                return res.status(500).json({ error: 'Нет предметов такой редкости' });
            craftedItem = {
                id: Date.now() + Math.random(),
            ...
            const resultCraftItem = await index_1.db.one(`
      ...
      `, [recipe.result_id]);
            if (!resultCraftItem)
                return res.status(500).json({ error: 'Результирующий ресурс не найден' });
            const existing = newInventory.find((i) => isCraftItem(i) && String(i.id) === String(recipe.result_id));
            if (existing) {
                existing.count += 1;
        ...
        await index_1.db.run(`UPDATE users SET inventory = ?, money = ?, craftCount = craftCount + 1, craftCreated = craftCreated + 1${factionInc} WHERE id = ?`, [JSON.stringify(newInventory), newMoney, userId]);
        (0, achievements_1.checkAchievement)(userId, 'craft').catch(() => { });
        (0, treasury_1.addToTreasury)(Math.floor(recipe.money_cost * 0.22), 'craft_recipe').catch(() => { });
        const u = await index_1.db.one('SELECT guildId FROM users WHERE id = ?', [userId]);
        if (u?.guildId) {
            (0, guild_1.updateGuildQuestProgress)(u.guildId, 'craft').catch(e => console.error('guildQuest craft:', e.message));
        }
        (0, events_1.markDirty)(userId, 'quests');
        // Туториал: первый крафт → шаг 3 (Арена)
        await index_1.db.run('UPDATE users SET tutorial_step = 3 WHERE id = ? AND tutorial_step = 2', [userId]);
        return res.json({ success: true, inventory: newInventory, moneyAfter: newMoney, item: craftedItem, message: 'Предмет создан!' });
    }
    else {
        await index_1.db.run('UPDATE users SET inventory = ?, money = ?, craftBroken = craftBroken + 1 WHERE id = ?', [JSON.stringify(newInventory), newMoney, userId]);
        (0, treasury_1.addToTreasury)(Math.floor(recipe.money_cost * 0.22), 'craft_recipe_fail').catch(() => { });
...
router.get('/craft/upgrade-info/:level/:rarity', async (req, res) => {
    ...
    const userId = req.userId;
    const data = await index_1.db.one('SELECT chance, money_cost FROM upgrade_chances WHERE level = ? AND rarity_id = ?', [level, rarity]);
    if (!data)
        return res.status(404).json({ error: 'Данные об уровне не найдены' });
    // Бонус фракции Ремесленник: +10% +1% за каждые 1000 крафтов
    const user = await index_1.db.one('SELECT faction, faction_craft_count FROM users WHERE id = ?', [userId]);
...
router.post('/craft/upgrade', async (req, res) => {
    ...
    const itemSlot = slots.find((s) => s && !isCraftItem(s));
    const stoneSlot = slots.find((s) => s && isCraftItem(s) && s.itemType === 'upgrade');
    ...
    const stoneIndex = inventory.findIndex((i) => isCraftItem(i) && i.id === stoneSlot.id && i.itemType === 'upgrade');
    if (stoneIndex === -1)
        return res.status(400).json({ error: 'Камень усиления не найден в инвентаре' });
    const stone = inventory[stoneIndex];
    if (stone.count < 1)
        return res.status(400).json({ error: 'Недостаточно камней усиления' });
    const upgradeData = await index_1.db.one('SELECT chance, money_cost FROM upgrade_chances WHERE level = ? AND rarity_id = ?', [targetLevel, itemSlot.rarity_id]);
    if (!upgradeData) {
    ...
    const STONE_BONUS = { 0: 0, 1: 5, 2: 10, 3: 15, 4: 20, 5: 30, 6: 50 };
    ...
    if (success) {
        ...
        const itemIdx = newInventory.findIndex((i) => i.id === itemSlot.id && !isCraftItem(i));
        if (itemIdx === -1) {
        ...
        if (ratingBonus > 0) {
            ...
            await index_1.db.run(`UPDATE users SET money = ?, inventory = ?, elo = ?, pveRating = pveRating + ?, craftCount = craftCount + 1, craftUpgraded = craftUpgraded + 1${upgradeFactionInc} WHERE id = ?`, [newMoney, JSON.stringify(newInventory), newElo, ratingBonus, userId]);
            (0, achievements_1.checkAchievement)(userId, 'craft').catch(() => { });
            (0, treasury_1.addToTreasury)(Math.floor(actualCost * 0.22), 'craft_upgrade').catch(() => { });
            const u = await index_1.db.one('SELECT guildId FROM users WHERE id = ?', [userId]);
            if (u?.guildId) {
                (0, guild_1.updateGuildQuestProgress)(u.guildId, 'craft').catch(e => console.error('guildQuest craft:', e.message));
            }
            ...
            if (targetLevel >= 7) {
                ...
                index_1.db.run('INSERT INTO chat_messages (id, senderId, targetId, content) VALUES (?, 0, NULL, ?)', [msgId, chatMsg.content]).catch(() => { });
        ...
        await index_1.db.run(`UPDATE users SET inventory = ?, money = ?, craftCount = craftCount + 1, craftUpgraded = craftUpgraded + 1${upgradeFactionInc2} WHERE id = ?`, [JSON.stringify(newInventory), newMoney, userId]);
        (0, achievements_1.checkAchievement)(userId, 'craft').catch(() => { });
        (0, treasury_1.addToTreasury)(Math.floor(actualCost * 0.22), 'craft_upgrade').catch(() => { });
        const u = await index_1.db.one('SELECT guildId FROM users WHERE id = ?', [userId]);
        if (u?.guildId) {
        ...
        if (targetLevel >= 7) {
            ...
            if (itemIdx !== -1) {
                ...
                const craftItem = await index_1.db.one(`
          ...
          `, [rarityId]);
                if (craftItem) {
                    const existingCraft = newInventory.find((i) => isCraftItem(i) && i.id === craftItem.id);
                    if (existingCraft) {
            ...
            await index_1.db.run('UPDATE users SET inventory = ?, money = ?, craftBroken = craftBroken + 1 WHERE id = ?', [JSON.stringify(newInventory), newMoney, userId]);
            (0, treasury_1.addToTreasury)(Math.floor(actualCost * 0.22), 'craft_upgrade_fail').catch(() => { });
            ...
            const brokenChatMsg = { id: brokenMsgId, senderId: 0, senderName: 'Глашатай', targetId: null,
                ...
                createdAt: new Date().toISOString() };
            index_1.db.run('INSERT INTO chat_messages (id, senderId, targetId, content) VALUES (?, 0, NULL, ?)', [brokenMsgId, brokenChatMsg.content]).catch(() => { });
            (0, events_1.broadcast)('message', { message: brokenChatMsg });
            return res.json({ success: false, inventory: newInventory, moneyAfter: newMoney, message: 'Неудача! Предмет разрушен.' });
        }
        else {
            // До +7 — просто неудача, предмет остаётся, камень и деньги списаны
            await index_1.db.run('UPDATE users SET inventory = ?, money = ? WHERE id = ?', [JSON.stringify(newInventory), newMoney, userId]);
            (0, treasury_1.addToTreasury)(Math.floor(actualCost * 0.22), 'craft_upgrade_fail').catch(() => { });
...
router.post('/craft/disassemble', async (req, res) => {
    ...
    if (!itemId)
        return res.status(400).json({ error: 'Укажите itemId' });
    const user = await index_1.db.one('SELECT * FROM users WHERE id = ?', [userId]);
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    const inventory = JSON.parse(user.inventory || '[]');
    const stoneIndex = inventory.findIndex((i) => isCraftItem(i) && String(i.id) === String(itemId) && i.itemType === 'upgrade');
    if (stoneIndex === -1)
        return res.status(400).json({ error: 'Руна улучшения не найдена' });
    const stone = inventory[stoneIndex];
    const rarityId = stone.rarity_id || 0;
    // Найти материал той же редкости (type = 'craft')
    const material = await index_1.db.one(`
        SELECT c.id, c.name, c.rarity_id, c.type, c.image,
               r.display_name as rarity_display, r.color as rarity_color
        FROM craft_items c
        JOIN rarities r ON c.rarity_id = r.id
    ...
    `, [rarityId]);
    if (!material)
        return res.status(400).json({ error: 'Нет материала такой редкости' });
    ...
    // Добавляем материал
    const existing = inventory.find((i) => isCraftItem(i) && i.id === material.id);
    if (existing) {
        existing.count += 1;
    }
    else {
...
const CURSE_STATS = { s: 'Сила', a: 'Ловкость', d: 'Защита', m: 'Мастерство' };
...
router.post('/craft/curse', async (req, res) => {
    ...
    if (!itemId || !crystalId)
        return res.status(400).json({ error: 'Укажите itemId и crystalId' });
    const user = await index_1.db.one('SELECT * FROM users WHERE id = ?', [userId]);
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    if (user.money < CURSE_COST)
        return res.status(400).json({ error: `Недостаточно серебра. Нужно ${CURSE_COST.toLocaleString()}` });
    const inventory = JSON.parse(user.inventory || '[]');
    const item = inventory.find((i) => i.id === itemId && !isCraftItem(i));
    if (!item)
        return res.status(400).json({ error: 'Предмет не найден в инвентаре' });
    const crystalIdx = inventory.findIndex((i) => isCraftItem(i) && i.id === crystalId && i.itemType === 'soul_crystal');
    if (crystalIdx === -1)
        return res.status(400).json({ error: 'Кристалл душ не найден в инвентаре' });
    ...
    const oldCurse = item.curseStat ? {
        ...
        statName: CURSE_STATS[item.curseStat] || item.curseStat,
    } : null;
    const curse = rollCurse();
    const statName = CURSE_STATS[curse.stat] || curse.stat;
    await index_1.db.run('UPDATE users SET inventory = ?, money = ?, craftCount = craftCount + 1 WHERE id = ?', [JSON.stringify(inventory), newMoney, userId]);
    (0, achievements_1.checkAchievement)(userId, 'craft').catch(() => { });
    res.json({
        oldCurse,
...
router.post('/craft/curse/apply', async (req, res) => {
    ...
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    const inventory = JSON.parse(user.inventory || '[]');
    const itemIdx = inventory.findIndex((i) => i.id === itemId && !isCraftItem(i));
    if (itemIdx === -1)
        return res.status(400).json({ error: 'Предмет не найден в инвентаре' });
    ...
    const message = keepOld
        ...
        : `Предмет проклят! +${curseData.value} к ${CURSE_STATS[curseData.stat] || curseData.stat} (ранг ${curseData.name})`;
    res.json({
        success: true,
        inventory,
