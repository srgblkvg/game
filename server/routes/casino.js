...
async function countTodayGames(gameType, userId) {
    const row = await index_1.db.one("SELECT COUNT(*) as cnt FROM casino_games WHERE user_id = ? AND game_type = ? AND created_at::date = CURRENT_DATE", [userId, gameType]);
    return row.cnt || 0;
}
// Получить активную игру пользователя
router.get('/casino/active', async (req, res) => {
    const userId = req.userId;
    const game = await index_1.db.one("SELECT * FROM casino_games WHERE user_id = ? AND status = 'playing' ORDER BY created_at DESC LIMIT 1", [userId]);
    ...
    if (!game)
        return res.json({ game: null, todayGames: todayBJ, dailyLimit: DAILY_LIMIT, remaining });
    // Не показываем вторую карту дилера до конца игры
    const dealerCards = JSON.parse(game.dealer_cards || '[]');
    const firstDealerCard = dealerCards.length > 0 ? dealerCards[0] : '??';
    const shownDealer = dealerCards.length > 0 ? [firstDealerCard, '??'] : [];
    res.json({
        game: {
            id: game.id,
...
router.post('/casino/blackjack/start', async (req, res) => {
    ...
    const active = await index_1.db.one("SELECT id FROM casino_games WHERE user_id = ? AND status = 'playing' LIMIT 1", [userId]);
    if (active)
        return res.status(400).json({ error: 'У вас уже есть активная игра. Завершите её.' });
    // Проверить дневной лимит
    const todayCount = await countTodayGames('blackjack', userId);
    if (todayCount >= DAILY_LIMIT)
        return res.status(400).json({ error: `Дневной лимит исчерпан (${todayCount}/${DAILY_LIMIT}). Возвращайтесь завтра!` });
    // Проверить деньги
    try {
        await index_1.db.tx(async (client) => {
            ...
            const gameResult = await client.query(`INSERT INTO casino_games (user_id, game_type, bet, status, player_cards, dealer_cards, deck, player_score, dealer_score)
                 VALUES ($1, 'blackjack', $2, $3, $4, $5, $6, $7, $8)
                 RETURNING id`, [userId, bet, status, JSON.stringify(pCards), JSON.stringify(dCards), JSON.stringify(finalDeck), pScore.score, dScore.score]);
            if (payout > 0) {
            ...
            if (status !== 'playing') {
                ...
                await client.query('UPDATE users SET casino_games_played = casino_games_played + 1, casino_won = casino_won + $1, casino_lost = casino_lost + $2 WHERE id = $3', [_bjwon, _bjlost, userId]);
                (0, achievements_1.checkAchievement)(userId, 'casino').catch(() => { });
            }
            res.json({
        ...
        if (e.message === 'no_money')
            return res.status(400).json({ error: `Недостаточно серебра. Нужно ${bet}, проверьте баланс` });
        throw e;
    }
});
// Ход — взять карту
router.post('/casino/blackjack/hit', async (req, res) => {
    ...
    try {
        const result = await index_1.db.tx(async (client) => {
            const game = (await client.query("SELECT * FROM casino_games WHERE user_id = $1 AND status = 'playing' ORDER BY created_at DESC LIMIT 1 FOR UPDATE", [userId])).rows[0];
            if (!game)
            ...
            if (pScore.score > 21) {
                // Bust
                await client.query("UPDATE casino_games SET player_cards = $1, deck = $2, player_score = $3, status = 'dealer_won', finished_at = NOW() WHERE id = $4", [JSON.stringify(playerCards), JSON.stringify(newDeck), pScore.score, game.id]);
                return {
            ...
            await client.query('UPDATE casino_games SET player_cards = $1, deck = $2, player_score = $3 WHERE id = $4', [JSON.stringify(playerCards), JSON.stringify(newDeck), pScore.score, game.id]);
            return {
                status: 'playing',
        ...
        if (e.message === 'no_game')
            return res.status(400).json({ error: 'Нет активной игры' });
        if (e.message === 'no_cards')
            return res.status(500).json({ error: 'В колоде закончились карты' });
        throw e;
    }
});
// Стоп — дилер добирает
router.post('/casino/blackjack/stand', async (req, res) => {
    ...
    try {
        const result = await index_1.db.tx(async (client) => {
            ...
            await client.query("UPDATE casino_games SET dealer_cards = $1, deck = $2, dealer_score = $3, player_score = $4, status = $5, finished_at = NOW() WHERE id = $6", [JSON.stringify(dealerCards), JSON.stringify(deck), dScore.score, pScore.score, status, game.id]);
            if (payout > 0) {
                await client.query('UPDATE users SET money = money + $1 WHERE id = $2', [payout, userId]);
            }
            // Статистика казино
            const { won: _swon, lost: _slost } = getCasinoStatUpdate(status, game.bet, payout);
            await client.query('UPDATE users SET casino_games_played = casino_games_played + 1, casino_won = casino_won + $1, casino_lost = casino_lost + $2 WHERE id = $3', [_swon, _slost, userId]);
            (0, achievements_1.checkAchievement)(userId, 'casino').catch(() => { });
            return {
                status, result,
                player_cards: playerCards, dealer_cards: dealerCards,
...
router.post('/casino/blackjack/double', async (req, res) => {
    ...
    try {
        const result = await index_1.db.tx(async (client) => {
            ...
            if (pScore.score > 21) {
                // Bust
                await client.query("UPDATE casino_games SET player_cards = $1, deck = $2, player_score = $3, bet = $4, status = 'dealer_won', finished_at = NOW() WHERE id = $5", [JSON.stringify(playerCards), JSON.stringify(deck), pScore.score, newBet, game.id]);
                return {
                    status: 'dealer_won', result: 'bust',
                    player_cards: playerCards,
            ...
            await client.query("UPDATE casino_games SET player_cards = $1, dealer_cards = $2, deck = $3, player_score = $4, dealer_score = $5, bet = $6, status = $7, finished_at = NOW() WHERE id = $8", [JSON.stringify(playerCards), JSON.stringify(dealerCards), JSON.stringify(deck), pScore.score, dScore.score, newBet, status, game.id]);
            if (payout > 0) {
                await client.query('UPDATE users SET money = money + $1 WHERE id = $2', [payout, userId]);
            }
            // Статистика казино
            const { won: _dwon, lost: _dlost } = getCasinoStatUpdate(status, newBet, payout);
            await client.query('UPDATE users SET casino_games_played = casino_games_played + 1, casino_won = casino_won + $1, casino_lost = casino_lost + $2 WHERE id = $3', [_dwon, _dlost, userId]);
            (0, achievements_1.checkAchievement)(userId, 'casino').catch(() => { });
            return {
                status, result,
                player_cards: playerCards, dealer_cards: dealerCards,
        ...
        if (e.message === 'not_allowed')
            return res.status(400).json({ error: 'Удвоение доступно только на первых двух картах' });
        if (e.message === 'no_money')
            return res.status(400).json({ error: 'Недостаточно серебра для удвоения. Проверьте баланс' });
        if (e.message === 'no_cards')
            return res.status(500).json({ error: 'В колоде закончились карты' });
        throw e;
    }
...
router.post('/casino/blackjack/surrender', async (req, res) => {
    ...
    try {
        const result = await index_1.db.tx(async (client) => {
            ...
            await client.query("UPDATE casino_games SET status = 'surrender', finished_at = NOW() WHERE id = $1", [game.id]);
            if (returnAmount > 0) {
            ...
            await client.query('UPDATE users SET casino_games_played = casino_games_played + 1, casino_won = casino_won + $1, casino_lost = casino_lost + $2 WHERE id = $3', [_surwon, _surlost, userId]);
            (0, achievements_1.checkAchievement)(userId, 'casino').catch(() => { });
            const dealerCards = JSON.parse(game.dealer_cards || '[]');
            return {
        ...
        if (e.message === 'no_game')
            return res.status(400).json({ error: 'Нет активной игры' });
        if (e.message === 'not_allowed')
            return res.status(400).json({ error: 'Сдаться можно только на первых двух картах' });
        throw e;
    }
});
...
function getCasinoStatUpdate(status, bet, payout) {
    if (status === 'player_won')
        return { won: payout - bet, lost: 0 };
    if (status === 'dealer_won')
        return { won: 0, lost: bet };
    if (status === 'surrender')
        return { won: 0, lost: bet - payout };
