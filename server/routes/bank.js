...
router.get('/bank', async (req, res) => {
    ...
    let user = await index_1.db.one('SELECT money, bank, accountNumber FROM users WHERE id = ?', [userId]);
    if (!user)
    ...
    if (!user.accountNumber) {
        ...
        let accNum = '';
        for (let i = 0; i < 6; i++)
            accNum += chars[Math.floor(Math.random() * chars.length)];
        await index_1.db.run('UPDATE users SET accountNumber = ? WHERE id = ?', [accNum, userId]);
...
router.post('/bank/deposit', async (req, res) => {
    ...
    try {
        const updated = await index_1.db.tx(async (client) => {
            ...
            (0, treasury_1.addToTreasury)(commission, 'bank_deposit').catch(() => { });
            return (await client.query('SELECT money, bank FROM users WHERE id = $1', [userId])).rows[0];
        });
        res.json({ success: true, pocket: updated.money, bank: updated.bank, commission, deposited: depositAmount });
        (0, events_1.sendToUser)(userId, { type: 'balance', money: updated.money, bank: updated.bank });
    }
    catch (e) {
...
router.post('/bank/withdraw', async (req, res) => {
    ...
    try {
        const updated = await index_1.db.tx(async (client) => {
            const user = (await client.query('SELECT bank FROM users WHERE id = $1', [userId])).rows[0];
            if (!user)
                throw new Error('User not found');
            if (user.bank < amount)
                throw new Error(`Недостаточно серебра в банке. Нужно ${amount}, есть ${user.bank}`);
...
router.post('/bank/transfer', async (req, res) => {
    ...
    try {
        const { updated, target, commission, receivedAmount } = await index_1.db.tx(async (client) => {
            // Проверяем существование получателя ДО списания
            const target = (await client.query('SELECT id, username, accountNumber FROM users WHERE accountNumber = $1', [accountNumber])).rows[0];
            if (!target)
                throw new Error('Счёт не найден');
            if (target.id === userId)
                throw new Error('Нельзя перевести самому себе');
            // Проверяем и списываем с банка отправителя
            let sender = (await client.query('SELECT bank, accountnumber FROM users WHERE id = $1', [userId])).rows[0];
            if (!sender)
                throw new Error('User not found');
            ...
            await client.query('INSERT INTO transfers (fromUserId, toUserId, fromAccount, toAccount, toUsername, amount, commission, received) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', [userId, target.id, sender.accountnumber, target.accountnumber, target.username, transferAmount, commission, receivedAmount]);
            (0, treasury_1.addToTreasury)(commission, 'bank_transfer').catch(() => { });
            const updated = (await client.query('SELECT bank FROM users WHERE id = $1', [userId])).rows[0];
...
router.get('/bank/transfers', async (req, res) => {
    ...
    if (filter === 'in')
        ...
        query += '(fromUserId = ? OR toUserId = ?)';
    query += ' ORDER BY id DESC LIMIT ?';
    const params = filter === 'all' ? [userId, userId, limit] : [userId, limit];
...
router.get('/bank/operations', async (req, res) => {
    ...
    query += ' ORDER BY id DESC LIMIT ?';
    const result2 = await index_1.db.query(query, [userId, limit]);
    res.json(result2.map((r) => ({
        ...r,
        createdAt: typeof r.createdAt === 'string' && /^\d+$/.test(r.createdAt) ? Number(r.createdAt) : (r.createdAt ? Math.floor(new Date(r.createdAt).getTime() / 1000) : r.createdAt)
    })));
});
