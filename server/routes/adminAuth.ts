...
router.get('/admin/check', async (req, res) => {
    const admin = await db.one('SELECT id FROM admins LIMIT 1', []);
...
router.post('/admin/register', async (req, res) => {
    ...
    const existingAdmin = await db.one('SELECT id FROM admins LIMIT 1', []);
    if (existingAdmin) return res.status(400).json({ error: 'Администратор уже существует' });

    const passwordHash = bcrypt.hashSync(password, 10);
    await db.run('INSERT INTO admins (username, passwordHash) VALUES (?, ?)', [username, passwordHash]);
