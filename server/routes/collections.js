...
router.get('/collections', async (req, res) => {
    ...
    const upgradeLevel = parseInt(req.query.upgradelevel) || 0;
    const isPlusTab = upgradeLevel >= 7;
    const items = await index_1.db.query(isPlusTab
        ? 'SELECT itemName, slot, rarity_id, upgradelevel FROM collections WHERE userId = ? AND upgradelevel >= 7'
        : 'SELECT itemName, slot, rarity_id, upgradelevel FROM collections WHERE userId = ? AND upgradelevel < 7', [userId]);
    // Сеты и их статус — фильтруем по upgradeLevel
    const sets = await index_1.db.query(`
    ...
    for (const row of sets) {
        ...
        if (row.item_name) {
            ...
            let bonuses = typeof row.bonuses === 'string' ? JSON.parse(row.bonuses || '{}') : (row.bonuses || {});
...
router.get('/collections/set-items', async (req, res) => {
    const items = await index_1.db.query('SELECT item_name, slot FROM collection_set_items');
    res.json(items);
});
...
router.post('/collections/add', async (req, res) => {
    ...
    const existing = await index_1.db.one(`SELECT id FROM collections WHERE userId = ? AND itemName = ? AND slot = ? AND rarity_id = ? AND upgradelevel ${targetLevel >= 7 ? '>=' : '<'} 7`, [userId, itemName, slot, rarityId || 0]);
    if (existing) {
        return res.status(400).json({ error: 'Предмет уже в коллекции' });
    }
    // Удаляем из инвентаря — prefer unlocked, prefer exact itemId
    const user = await index_1.db.one('SELECT inventory FROM users WHERE id = ?', [userId]);
    if (!user)
    ...
    if (itemId !== undefined) {
        itemIndex = inventory.findIndex((item) => String(item.id) === String(itemId));
    }
    // Fallback: по имени + слоту, prefer unlocked
    if (itemIndex === -1) {
        const candidates = inventory
            .map((item, idx) => ({ item, idx }))
            .filter(({ item }) => item.name === itemName && item.slot === slot);
    ...
    await index_1.db.run('INSERT INTO collections (userId, itemName, slot, rarity_id, upgradelevel) VALUES (?, ?, ?, ?, ?)', [userId, itemName, slot, removed.rarity_id || 0, actualUpgrade]);
    (0, achievements_1.checkAchievement)(userId, 'collection').catch(() => { });
