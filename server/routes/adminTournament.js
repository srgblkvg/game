...
function divisionLabel(key) { return DIVISION_LABELS[key] || key; }
...
router.post('/tournaments', async (req, res) => {
    ...
    const existing = await index_1.db.one("SELECT id FROM tournaments WHERE division = ? AND status IN ('registration', 'in_progress')", [division]);
    if (existing)
        return res.status(400).json({ error: 'В этом дивизионе уже есть активный турнир' });
    const now = Math.floor(Date.now() / 1000);
    await index_1.db.run('INSERT INTO tournaments (division, status, registrationStart, registrationEnd, prizePool, createdAt) VALUES (?, ?, ?, ?, ?, ?)', [division, status || 'registration', registrationStart || now, registrationEnd || (now + 86400), prizePool || div.basePool, now]);
...
router.put('/tournaments/:id', async (req, res) => {
    ...
    const t = await index_1.db.one('SELECT * FROM tournaments WHERE id = ?', [id]);
    if (!t)
        return res.status(404).json({ error: 'Турнир не найден' });
    await index_1.db.run('UPDATE tournaments SET status=?, registrationStart=?, registrationEnd=?, prizePool=? WHERE id=?', [
        status || t.status,
...
router.delete('/tournaments/:id', async (req, res) => {
    const id = req.params.id;
    await index_1.db.run('DELETE FROM tournament_participants WHERE tournamentId = ?', [id]);
    await index_1.db.run('DELETE FROM tournament_matches WHERE tournamentId = ?', [id]);
    await index_1.db.run('DELETE FROM tournaments WHERE id = ?', [id]);
    res.json({ success: true });
});
...
router.post('/tournaments/:id/finish', async (req, res) => {
    ...
    if (!t)
        return res.status(404).json({ error: 'Турнир не найден' });
    await index_1.db.run('UPDATE tournaments SET status = ?, completedAt = ? WHERE id = ?', ['completed', new Date().toISOString(), id]);
...
router.post('/tournaments/:id/start', async (req, res) => {
    ...
    if (!t)
        return res.status(404).json({ error: 'Турнир не найден' });
    await index_1.db.run('UPDATE tournaments SET status = ? WHERE id = ?', ['in_progress', id]);
    res.json({ success: true, message: `Турнир «${divisionLabel(t.division)}» запущен` });
});
exports.default = router;
//# sourceMappingURL=adminTournament.js.map