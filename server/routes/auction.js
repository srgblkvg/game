...
async function returnItemToInventory(userId, itemData) {
    ...
    if (isCraft) {
        const existingIdx = inventory.findIndex((i) => (i.type === 'craft_item' || i.type === 'material') && String(i.id) === String(itemData.id));
        if (existingIdx !== -1) {
    ...
    if (!isCraft && equipCount >= maxSlots) {
        await (0, overflow_1.addToOverflow)(userId, { ...itemData, count });
    }
    else {
...
index_1.db.run(`CREATE TABLE IF NOT EXISTS auction_history (
...
index_1.db.run(`ALTER TABLE users ADD COLUMN IF NOT EXISTS auction_sales INTEGER DEFAULT 0`).catch(() => { });
index_1.db.run(`ALTER TABLE users ADD COLUMN IF NOT EXISTS overflowmoney INTEGER DEFAULT 0`).catch(() => { });
// Мин. цены по редкости
const priceFloor = { 0: 5, 1: 20, 2: 100, 3: 400, 4: 1500, 5: 6000, 6: 20000 };
...
router.get('/auction/similar', async (req, res) => {
    ...
    const upgLevel = parseInt(req.query.upgradeLevel) || 0;
    const sellCount = parseInt(req.query.sellCount) || 1;
    if (!name)
        return res.json({ count: 0 });
    const now = Math.floor(Date.now() / 1000);
    const lots = await index_1.db.query(`SELECT l.startPrice, l.buyoutPrice, l.currentBid, l.itemData
    ...
    const getPerUnit = (total, itemDataJson) => {
        try {
...
router.get('/auction', async (req, res) => {
    ...
    const expired = await index_1.db.query('SELECT * FROM auction_lots WHERE endsAt <= ? AND currentBidderId IS NOT NULL', [now]);
    for (const lot of expired) {
        ...
        await index_1.db.run('UPDATE users SET overflowmoney = COALESCE(overflowmoney, 0) + ?, auctionTrades = auctionTrades + 1 WHERE id = ?', [payout, lot.sellerId]);
        (0, achievements_1.checkAchievement)(lot.sellerId, 'auction').catch(() => { });
        // Отдать предмет покупателю — всегда на склад
        const buyItemData = JSON.parse(lot.itemData);
        await (0, overflow_1.addToOverflow)(lot.currentBidderId, buyItemData);
        // Запись в историю
        await index_1.db.run(`INSERT INTO auction_history (sellerId, buyerId, itemName, itemData, price, commission, createdAt)
            VALUES (?, ?, ?, ?, ?, ?, ?)`, [lot.sellerId, lot.currentBidderId, JSON.parse(lot.itemData).name || 'Предмет', lot.itemData, lot.currentBid, commission, new Date().toISOString()]);
        (0, treasury_1.addToTreasury)(commission, 'auction_expired').catch(() => { });
        // Уведомления
        const buyerName = (await index_1.db.one('SELECT username FROM users WHERE id = ?', [lot.currentBidderId]))?.username || 'Кто-то';
        (0, events_1.pushNotification)(lot.sellerId, { type: 'auction_sold', message: `${buyerName} купил «${JSON.parse(lot.itemData).name || 'Предмет'}» за ${lot.currentBid} серебра` });
        (0, events_1.sendToUser)(lot.sellerId, { type: 'auction_badge', count: 1 });
        await index_1.db.run('UPDATE users SET auction_sales = COALESCE(auction_sales, 0) + 1 WHERE id = ?', [lot.sellerId]);
        (0, events_1.pushNotification)(lot.currentBidderId, { type: 'system', message: `Вы выиграли «${JSON.parse(lot.itemData).name || 'Предмет'}» на аукционе!` });
        await index_1.db.run('DELETE FROM auction_lots WHERE id = ?', [lot.id]);
        await index_1.db.run('DELETE FROM chat_messages WHERE item_data LIKE ?', [`%"lotId":${lot.id}%`]);
        (0, events_1.broadcast)('auction_message_removed', { lotId: lot.id });
    }
    // Возвращаем непроданные лоты продавцам — на склад
    const unsold = await index_1.db.query('SELECT * FROM auction_lots WHERE endsAt <= ? AND currentBidderId IS NULL', [now]);
    for (const lot of unsold) {
        const itemData = JSON.parse(lot.itemData);
        await (0, overflow_1.addToOverflow)(lot.sellerId, itemData);
        (0, events_1.pushNotification)(lot.sellerId, { type: 'system', message: `Лот «${JSON.parse(lot.itemData).name || 'Предмет'}» не был продан и возвращён на склад` });
        await index_1.db.run('DELETE FROM auction_lots WHERE id = ?', [lot.id]);
        await index_1.db.run('DELETE FROM chat_messages WHERE item_data LIKE ?', [`%"lotId":${lot.id}%`]);
        (0, events_1.broadcast)('auction_message_removed', { lotId: lot.id });
    }
    const lots = await index_1.db.query(`
    ...
    const sort = req.query.sort || 'end';
    const groupFilter = req.query.group || '';
    let filtered = allLots;
    if (search) {
        const q = search.toLowerCase();
        filtered = filtered.filter((l) => (l.itemData?.name || '').toLowerCase().includes(q));
    }
    if (category && category !== 'all') {
        filtered = filtered.filter((l) => {
            ...
            if (category === 'shield')
                return slot === 'shield';
            if (category === 'armor')
                return ['helmet', 'chest', 'gloves', 'boots'].includes(slot);
            if (category === 'accessory')
                return ['amulet', 'ring', 'belt'].includes(slot);
            if (category === 'material')
                return l.itemData?.type === 'craft_item' || l.itemData?.type === 'material';
            return slot === category;
        });
    }
    // Stat filters
    const statMap = { minStr: 's', minAgi: 'a', minDef: 'd', minMag: 'm' };
    for (const [clientKey, statKey] of Object.entries(statMap)) {
        const minVal = parseInt(req.query[clientKey]) || 0;
        if (minVal > 0) {
            filtered = filtered.filter((l) => (l.itemData?.bonuses?.[statKey] || 0) >= minVal);
        }
    }
    // Sort
    if (sort === 'price_asc')
    ...
    for (const lot of filtered) {
        ...
        if (existing) {
            existing.count++;
            if (bid < existing.minBid)
                existing.minBid = bid;
            if (buyout !== null && (existing.minBuyout === null || buyout < existing.minBuyout))
                existing.minBuyout = buyout;
        }
        else {
            groupsMap.set(key, {
                item: lot.itemData,
    ...
    const groupPage = parseInt(req.query.groupPage) || 1;
    ...
    const myLotCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM auction_lots WHERE sellerId = ? AND endsat > ?', [req.userId, Math.floor(Date.now() / 1000)])).cnt;
...
router.post('/auction/sell', async (req, res) => {
    ...
    if (startPrice < floor)
        return res.status(400).json({ error: `Мин. цена за 1 шт для этой редкости: ${floor} серебра` });
    if (buyoutPrice && buyoutPrice <= startPrice)
        return res.status(400).json({ error: 'Цена выкупа должна быть выше стартовой' });
    // Проверка лимита (10 лотов, 20 с премиумом)
    const userLotCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM auction_lots WHERE sellerId = ? AND endsat > ?', [userId, Math.floor(Date.now() / 1000)])).cnt;
    const user = await index_1.db.one('SELECT money, inventory, premiumUntil FROM users WHERE id = ?', [userId]);
    const hasPremium = (user.premiumUntil || 0) > Math.floor(Date.now() / 1000);
    const maxLots = hasPremium ? 20 : 10;
    if (userLotCount >= maxLots)
        return res.status(400).json({ error: `Максимум ${maxLots} лотов` });
    // Комиссия за листинг 5% (от общей стартовой цены)
    const listingFee = Math.max(1, Math.floor(totalStartPrice * 0.05));
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    if (user.money < listingFee)
        return res.status(400).json({ error: `Недостаточно монет для листинга (${listingFee} серебра)` });
    // Убираем предмет из инвентаря
    const inventory = JSON.parse(user.inventory || '[]');
    const idx = inventory.findIndex((i) => String(i.id) === String(itemData.id));
    if (idx === -1)
    ...
    if (isMaterial) {
        ...
        if (itemCount > availableCount)
            return res.status(400).json({ error: `Недостаточно: есть ${availableCount}, выбрано ${itemCount}` });
        if (itemCount >= availableCount) {
            // Продаём весь стек
    ...
    await index_1.db.run('UPDATE users SET money = money - ?, inventory = ? WHERE id = ?', [listingFee, JSON.stringify(inventory), userId]);
    const lotResult = await index_1.db.run(`INSERT INTO auction_lots (sellerId, itemData, startPrice, buyoutPrice, currentBid, duration, endsAt, createdAt)
    ...
    (0, treasury_1.addToTreasury)(listingFee, 'auction_listing').catch(() => { });
    // Системное сообщение в чат (вкладка Аукцион)
    const sellerName = (await index_1.db.one('SELECT username FROM users WHERE id = ?', [userId]))?.username || 'Кто-то';
    ...
    const chatInfo = await index_1.db.run('INSERT INTO chat_messages (senderId, targetId, content, item_data, senderguild, senderguildid) VALUES (?, NULL, ?, ?, NULL, NULL)', [0, `📦 ${sellerName} выставил лот`, auctionItemData]);
    const chatMsg = {
    ...
    (0, events_1.broadcast)('message', { message: chatMsg });
    (0, events_1.broadcast)('auction_changed', {});
...
router.post('/auction/bid', async (req, res) => {
    ...
    try {
        await index_1.db.tx(async (client) => {
            const now = Math.floor(Date.now() / 1000);
            const lot = (await client.query('SELECT * FROM auction_lots WHERE id = $1 AND endsAt > $2 FOR UPDATE', [lotId, now])).rows[0];
            if (!lot)
                throw new Error('Лот не найден или истёк');
            if (lot.sellerid === userId)
                throw new Error('Нельзя ставить на свой лот');
            ...
            if (amount < minBid)
                throw new Error(`Мин. ставка: ${minBid} серебра`);
            const user = (await client.query('SELECT money FROM users WHERE id = $1', [userId])).rows[0];
        ...
        (0, events_1.broadcast)('auction_changed', { lotId });
        // Системное сообщение о перебивке ставки
        const lot = await index_1.db.one('SELECT * FROM auction_lots WHERE id = ?', [lotId]);
        if (lot) {
            const itemData = JSON.parse(lot.itemdata || lot.itemData || '{}');
            const bidderName = (await index_1.db.one('SELECT username FROM users WHERE id = ?', [userId]))?.username || 'Кто-то';
            const prevBidderId = lot.currentbidderid;
            const previousBidderName = (prevBidderId && prevBidderId !== userId)
                ? ((await index_1.db.one('SELECT username FROM users WHERE id = ?', [prevBidderId]))?.username || 'Кто-то')
                : null;
            const auctionItemData = JSON.stringify({
                type: 'auction_bid',
                lotId,
                ...
                previousBidderName,
                sellerName: (await index_1.db.one('SELECT username FROM users WHERE id = ?', [lot.sellerid || lot.sellerId]))?.username || 'Кто-то',
                endsAt: lot.endsat || lot.endsAt,
                createdAt: Math.floor(Date.now() / 1000),
            });
            const chatInfo = await index_1.db.run('INSERT INTO chat_messages (senderId, targetId, content, item_data, senderguild, senderguildid) VALUES (?, NULL, ?, ?, NULL, NULL)', [0, `💰 ${bidderName} перебил ставку`, auctionItemData]);
            const chatMsg = {
                id: chatInfo.lastInsertRowid,
                senderId: 0,
                senderName: 'Глашатай',
...
router.post('/auction/buyout', async (req, res) => {
    ...
    const now = Math.floor(Date.now() / 1000);
    const lot = await index_1.db.one('SELECT * FROM auction_lots WHERE id = ? AND endsAt > ?', [lotId, now]);
    if (!lot)
        return res.status(404).json({ error: 'Лот не найден' });
    if (!lot.buyoutPrice)
        return res.status(400).json({ error: 'У лота нет выкупа' });
    if (lot.sellerId === userId)
        return res.status(400).json({ error: 'Нельзя купить свой лот' });
    const user = await index_1.db.one('SELECT money, inventory FROM users WHERE id = ?', [userId]);
    if (user.money < lot.buyoutPrice)
    ...
    if (lot.currentBidderId) {
        await index_1.db.run('UPDATE users SET overflowmoney = COALESCE(overflowmoney, 0) + ? WHERE id = ?', [lot.currentBid, lot.currentBidderId]);
    }
    const itemData = JSON.parse(lot.itemData);
    // Добавляем предмет покупателю — всегда на склад
    await (0, overflow_1.addToOverflow)(userId, itemData);
    await index_1.db.run('UPDATE users SET money = money - ?, auctionTrades = auctionTrades + 1 WHERE id = ?', [lot.buyoutPrice, userId]);
    // Продавец получает на склад (нельзя ограбить)
    await index_1.db.run('UPDATE users SET overflowmoney = COALESCE(overflowmoney, 0) + ?, auctionTrades = auctionTrades + 1 WHERE id = ?', [payout, lot.sellerId]);
    (0, achievements_1.checkAchievement)(lot.sellerId, 'auction').catch(() => { });
    await index_1.db.run('DELETE FROM auction_lots WHERE id = ?', [lotId]);
    await index_1.db.run('DELETE FROM chat_messages WHERE item_data LIKE ?', [`%"lotId":${lotId}%`]);
    (0, events_1.broadcast)('auction_message_removed', { lotId });
    // Daily quests — track auction trades
    (0, events_1.markDirty)(userId, 'quests');
    (0, events_1.markDirty)(lot.sellerId, 'quests');
    // Запись в историю
    const buyItemData = JSON.parse(lot.itemData);
    await index_1.db.run(`INSERT INTO auction_history (sellerId, buyerId, itemName, itemData, price, commission, createdAt)
        VALUES (?, ?, ?, ?, ?, ?, ?)`, [lot.sellerId, userId, buyItemData.name || 'Предмет', lot.itemData, lot.buyoutPrice, commission, new Date().toISOString()]);
    (0, treasury_1.addToTreasury)(commission, 'auction_buyout').catch(() => { });
    // Уведомление продавцу — прямой WS + toast
    const buyerName = (await index_1.db.one('SELECT username FROM users WHERE id = ?', [userId]))?.username || 'Кто-то';
    (0, events_1.pushNotification)(lot.sellerId, { type: 'auction_sold', message: `${buyerName} выкупил «${buyItemData.name || 'Предмет'}» за ${lot.buyoutPrice} серебра` });
    (0, events_1.sendToUser)(lot.sellerId, { type: 'auction_badge', count: 1 });
    await index_1.db.run('UPDATE users SET auction_sales = COALESCE(auction_sales, 0) + 1 WHERE id = ?', [lot.sellerId]);
    (0, events_1.broadcast)('auction_changed', {});
    ...
    const buyoutItemData = JSON.stringify({
        ...
        buyerName,
        sellerName: (await index_1.db.one('SELECT username FROM users WHERE id = ?', [lot.sellerId]))?.username || 'Кто-то',
    });
    const buyoutChatInfo = await index_1.db.run('INSERT INTO chat_messages (senderId, targetId, content, item_data, senderguild, senderguildid) VALUES (?, NULL, ?, ?, NULL, NULL)', [0, `✅ ${buyerName} выкупил лот за ${lot.buyoutPrice} серебра`, buyoutItemData]);
    const buyoutChatMsg = {
    ...
    (0, events_1.broadcast)('message', { message: buyoutChatMsg });
    res.json({ success: true });
});
...
router.post('/auction/buy-partial', async (req, res) => {
    ...
    const lot = await index_1.db.one('SELECT * FROM auction_lots WHERE id = ? AND endsAt > ?', [lotId, now]);
    if (!lot)
        return res.status(404).json({ error: 'Лот не найден или истёк' });
    if (lot.sellerId === userId)
        return res.status(400).json({ error: 'Нельзя купить свой лот' });
    const itemData = JSON.parse(lot.itemData);
    const stackCount = itemData.count || 1;
    if (stackCount <= 1)
        return res.status(400).json({ error: 'Этот лот нельзя купить частично' });
    if (quantity > stackCount)
        return res.status(400).json({ error: `В лоте только ${stackCount} шт.` });
    ...
    if (!user || user.money < cost)
        return res.status(400).json({ error: `Недостаточно серебра. Нужно ${cost}, есть ${user?.money || 0}` });
    ...
    await (0, overflow_1.addToOverflow)(userId, { ...itemData, count: quantity });
    ...
    if (remainingCount <= 0) {
        ...
        await index_1.db.run(`UPDATE auction_lots SET itemData = ?, startPrice = ?, buyoutPrice = ?, currentBid = ? WHERE id = ?`, [JSON.stringify(newItemData), newStartPrice, newBuyoutPrice, newCurrentBid, lotId]);
        ...
        if (lot.currentBidderId && newCurrentBid && newCurrentBid < lot.currentBid) {
            ...
            await index_1.db.run('UPDATE users SET money = money + ? WHERE id = ?', [refund, lot.currentBidderId]);
        }
    }
    // Списываем деньги покупателю и начисляем продавцу на склад
    await index_1.db.run('UPDATE users SET money = money - ?, auctionTrades = auctionTrades + 1 WHERE id = ?', [cost, userId]);
    await index_1.db.run('UPDATE users SET overflowmoney = COALESCE(overflowmoney, 0) + ?, auctionTrades = auctionTrades + 1 WHERE id = ?', [payout, lot.sellerId]);
    (0, achievements_1.checkAchievement)(lot.sellerId, 'auction').catch(() => { });
    // Daily quests — track auction trades
    (0, events_1.markDirty)(userId, 'quests');
    ...
    // Запись в историю
    await index_1.db.run(`INSERT INTO auction_history (sellerId, buyerId, itemName, itemData, price, commission, createdAt)
        VALUES (?, ?, ?, ?, ?, ?, ?)`, [lot.sellerId, userId, itemData.name || 'Предмет', JSON.stringify({ ...itemData, count: quantity }), cost, commission, new Date().toISOString()]);
    (0, treasury_1.addToTreasury)(commission, 'auction_partial').catch(() => { });
    (0, events_1.broadcast)('auction_changed', {});
...
router.post('/auction/cancel', async (req, res) => {
    ...
    const { lotId } = req.body;
    if (!lotId)
        return res.status(400).json({ error: 'Укажите lotId' });
    const now = Math.floor(Date.now() / 1000);
    const lot = await index_1.db.one('SELECT * FROM auction_lots WHERE id = ? AND endsAt > ?', [lotId, now]);
    if (!lot)
        return res.status(404).json({ error: 'Лот не найден или истёк' });
    if (lot.sellerId !== userId)
        return res.status(400).json({ error: 'Это не ваш лот' });
    const itemData = JSON.parse(lot.itemData);
...
router.post('/auction/reset-badge', async (req, res) => {
    const userId = req.userId;
    await index_1.db.run('UPDATE users SET auction_sales = 0 WHERE id = ?', [userId]);
    res.json({ success: true });
});
// История сделок
router.get('/auction/history', async (req, res) => {
    ...
    const totalRow = await index_1.db.one('SELECT COUNT(*) as cnt FROM auction_history', []);
    const total = totalRow?.cnt || 0;
    const history = await index_1.db.query(`
...
router.get('/auction/price-history', async (req, res) => {
    ...
    const rows = await index_1.db.query(`
        SELECT
