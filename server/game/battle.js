...
exports.dodgeChance = dodgeChance;
exports.critChance = critChance;
exports.critMult = critMult;
exports.blockChance = blockChance;
exports.blockReduction = blockReduction;
exports.counterChance = counterChance;
...
function dodgeChance(defStats, atkStats) {
    const defW = (0, stats_2.sv)(defStats, stats_2.F.dodgeDef);
    const atkW = (0, stats_2.sv)(atkStats, stats_2.F.dodgePen);
    const extra = defStats.extra || {};
    const extraDodge = (extra.dodge || 0);
    const luckBonus = (defStats.luckBoost || 0) / 100;
    return Math.max(0, (defW / (defW + 500)) *
        (1 - atkW / (atkW + 250)) / 1.5 +
        Math.min(0.5, extraDodge / (extraDodge + 300))) + luckBonus;
}
function critChance(stats) {
    const ex = stats.extra || {};
    const extraCrit = (ex.crit || 0);
    const luckBonus = (stats.luckBoost || 0) / 100;
    return Math.min(0.8, (0, stats_2.sv)(stats, stats_2.F.crit) / ((0, stats_2.sv)(stats, stats_2.F.crit) + 500) / 1.5 + extraCrit / (extraCrit + 300)) + luckBonus;
}
function critMult(stats) {
    return 1.5 + 0.5 * ((0, stats_2.sv)(stats, stats_2.F.crit) / ((0, stats_2.sv)(stats, stats_2.F.crit) + 50));
}
function blockChance(defStats) {
    const extraBlock = (defStats.extra.fullBlock || 0);
    return Math.min(0.75, (0, stats_2.sv)(defStats, stats_2.F.block) / ((0, stats_2.sv)(defStats, stats_2.F.block) + 500) / 1.5 + extraBlock / (extraBlock + 300));
}
function blockReduction(defStats, atkStats) {
    const ratio = (0, stats_2.sv)(defStats, stats_2.F.block) / Math.max(1, (0, stats_2.sv)(atkStats, stats_2.F.damage));
    return Math.min(0.75, 0.9 * ratio);
}
function counterChance(defStats, atkStats, extraBonus) {
    const dSum = (0, stats_2.sv)(defStats, stats_2.F.counterDef) + (0, stats_2.sv)(atkStats, stats_2.F.counterTgt);
    return Math.min(0.5, (dSum > 0 ? (0, stats_2.sv)(defStats, stats_2.F.counterDef) / dSum * 0.5 / 1.5 : 0) + extraBonus / (extraBonus + 300));
}
function stunChance(atkStats, defStats) {
    const sum = (0, stats_2.sv)(atkStats, stats_2.F.stunAtk) + (0, stats_2.sv)(defStats, stats_2.F.stunDef);
    return sum > 0 ? (0, stats_2.sv)(atkStats, stats_2.F.stunAtk) / sum * 0.3 : 0;
}
// Урон: мин = level, макс = сила (S)
function rollDamage(stats, level) {
    const S = stats.s;
...
function runTurn(ctx, addStep) {
    ...
    if (Math.random() < fullBlockChance) {
        ...
        if (blockPen > 0)
            blockReduce = Math.max(0, blockReduce * (1 - blockPen / 100));
...
function runBattle(attacker, defender) {
    const statsA = attacker.stats || (0, stats_1.currentStats)(attacker.base, attacker.equipment, attacker.drinkBonuses, attacker.collectionBonus, attacker.guildBonus);
    const statsD = defender.stats || (0, stats_1.currentStats)(defender.base, defender.equipment, defender.drinkBonuses, defender.collectionBonus, defender.guildBonus);
    ...
    const applyPoison = (name, actor, hp, maxHp) => {
        const poison = actor === 'attacker' ? poisonOnA : poisonOnD;
        if (!poison || poison.turnsLeft <= 0)
            return hp;
