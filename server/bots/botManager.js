...
exports.getBotsStatus = getBotsStatus;
// In-process bot manager — управление ботами из админки
const index_1 = require("../db/index");
const botNames_1 = require("./botNames");
const DIV_LABEL = {
...
index_1.db.run(`CREATE TABLE IF NOT EXISTS bot_accounts (
...
async function registerBot() {
    ...
    const botName = (0, botNames_1.pickBotName)();
    if (botName) {
        try {
            await index_1.db.run('UPDATE users SET username = ? WHERE id = ?', [botName, data.user.id]);
            data.user.username = botName;
        }
        catch { /* имя уже занято — оставляем гостевое */ }
    }
    await index_1.db.run('INSERT INTO bot_accounts (userId, username, token, active, createdAt) VALUES (?, ?, ?, 1, ?)', [data.user.id, data.user.username, data.token, new Date().toISOString()]);
    return { id: data.user.id, username: data.user.username, token: data.token, active: true };
}
async function getStoredBots() {
    const rows = await index_1.db.query('SELECT * FROM bot_accounts WHERE active = 1');
...
async function doManageEq(token, botId, state) {
    const char = await apiGet(token, '/character/me');
    if (!char || char.error || !char.inventory) {
        state.stats.lastActionResult = `Ошибка персонажа: ${char?.error || 'нет инвентаря'}`;
        return;
    }
    ...
    for (const slot of EQUIP_SLOTS) {
        ...
        const candidates = inv.filter((i) => {
            if (i.type === 'craft_item' || i.type === 'material')
                return false;
            const itemSlot = i.slot || '';
            if (slot.startsWith('weapon') && itemSlot.startsWith('weapon'))
                return true;
            if (slot.startsWith('ring') && itemSlot.startsWith('ring'))
                return true;
        ...
        if (itemScore(best) > currentScore) {
            try {
                ...
                const idx = inv.findIndex((i) => i.id === best.id);
                if (idx !== -1) {
                    if (current)
                        inv[idx] = current;
                    else
            ...
            catch (e) { /* слот не подходит — пропускаем */ }
        }
    }
    // 2. Продать на аукционе ненужное
    const now = Math.floor(Date.now() / 1000);
    const currentInv = (await apiGet(token, '/character/me')).inventory || [];
    const currentEq = (await apiGet(token, '/character/me')).equipment || {};
    for (const item of currentInv) {
        if (actions.length >= 3)
            break; // макс 3 действия за раз
        const isEquipped = Object.values(currentEq).some((e) => e?.id === item.id);
        if (isEquipped)
            continue;
...
async function ensureHp(token, botId) {
    const char = await apiGet(token, '/character/me');
    if (!char || char.error)
        return false;
    ...
    if (maxHp <= 0)
        return false;
    if (hp >= maxHp * 0.9)
        return true; // HP >= 90% — можно в бой
    if (char.money < 10)
        return true; // нет денег — рискуем
    const missing = maxHp - hp;
    const cost = missing * 2;
    // Есть деньги — лечимся мгновенно
    if (char.money >= cost) {
        try {
            await apiCall(token, 'POST', '/tavern/heal', { full: true });
        }
        catch { }
        return true;
    }
...
async function regenOrWork(token, botId, state, curHp, maxHp) {
    const char = await apiGet(token, '/character/me');
    ...
    if (regenSec > durations[durations.length - 1])
        bestDur = durations[durations.length - 1];
    try {
...
async function doPve(token, botId, state) {
    const [mobs, char] = await Promise.all([apiGet(token, '/mobs'), apiGet(token, '/character/me')]);
    if (!Array.isArray(mobs) || !mobs.length) {
        state.stats.lastActionResult = 'Нет мобов';
        return;
    }
    if (!char || char.error) {
        state.stats.lastActionResult = `Ошибка персонажа: ${char?.error || 'нет ответа'}`;
        return;
    }
    ...
    let pool = mobs.filter((m) => m.level <= lvl);
    if (!pool.length) {
        state.stats.lastActionResult = `Нет мобов ≤ ур.${lvl}`;
        return;
    }
    // Предпочитаем равных (60%), слабее допустимо (40%)
    const equal = pool.filter((m) => m.level === lvl);
    if (equal.length && Math.random() < 0.6)
        pool = equal;
    const mob = pool[Math.floor(Math.random() * pool.length)];
    if (!mob) {
        state.stats.lastActionResult = 'Моб не выбран';
        return;
    }
...
async function doPvp(token, botId, state) {
    const char = await apiGet(token, '/character/me');
    if (!char || char.error) {
        state.stats.lastActionResult = `Ошибка персонажа: ${char?.error || 'нет ответа'}`;
        return;
    ...
    if (!opp || !opp.stats) {
        state.stats.lastActionResult = 'PvP: нет соперников';
        return;
    }
    ...
    if (opp.level > char.level && char.money >= 10) {
        ...
        catch (e) { /* не вышло — атакуем что есть */ }
    }
    try {
...
// ─── Работа ───
async function doJob(token, botId, state) {
    const durations = [600, 1800, 3600];
...
async function doAuctionBuy(token, botId, state) {
    const char = await apiGet(token, '/character/me');
    if (!char || char.error) {
        state.stats.lastActionResult = `Ошибка персонажа: ${char?.error || 'нет ответа'}`;
        return;
    ...
    if (!Array.isArray(lots) || !lots.length) {
        state.stats.lastActionResult = 'Аукцион: нет лотов';
        return;
    }
    const eq = char.equipment || {};
    // Ищем предметы экипировки, которые лучше текущих
    for (const lot of lots) {
        if (lot.sellerId === char.id)
            continue;
        const itemData = typeof lot.itemData === 'string' ? JSON.parse(lot.itemData) : lot.itemData;
        if (!itemData || itemData.type === 'craft_item' || itemData.type === 'material')
            continue;
        if (!itemData.slot)
            continue;
        const slot = itemData.slot;
        // Находим подходящий слот (weapon → weapon1/weapon2, ring → ring1/ring2)
        const matchingSlots = EQUIP_SLOTS.filter(s => s === slot || (s.startsWith('weapon') && slot.startsWith('weapon')) || (s.startsWith('ring') && slot.startsWith('ring')));
        ...
        if (lotScore > currentScore) {
            ...
            if (price <= char.money * 0.4) {
                ...
                catch (e) { /* недостаточно денег — смотрим дальше */ }
            }
        }
    }
    // Если ничего не подошло — случайная ставка на дешёвый лот
    const cheapLots = lots.filter((l) => {
    ...
    if (cheapLots.length && Math.random() < 0.3) {
        ...
        try {
            ...
            state.stats.lastActionResult = `Аукцион: ${e.message}`;
        }
    }
    else {
...
// ─── Крафт ───
async function doCraft(token, botId, state) {
    const recipes = await apiGet(token, '/craft/recipes');
    if (!Array.isArray(recipes) || !recipes.length) {
        state.stats.lastActionResult = 'Крафт: нет рецептов';
        return;
    }
    // Сортируем по редкости результата (лучшие сначала)
    const sorted = [...recipes].sort((a, b) => (b.result?.rarity_id || 0) - (a.result?.rarity_id || 0));
...
async function doTournament(token, botId, state) {
    const char = await apiGet(token, '/character/me');
    if (!char || char.error) {
        state.stats.lastActionResult = 'Нет персонажа для турнира';
        return;
    }
    const tRes = await apiGet(token, '/tournament?tab=active');
    const tournaments = tRes.tournaments || [];
    if (!tournaments.length) {
        state.stats.lastActionResult = 'Турниров нет';
        return;
    }
    const now = Math.floor(Date.now() / 1000);
    ...
    const candidates = tournaments.filter((t) => {
        ...
        if (t.myRegistration)
            return false;
        // Проверка уровня
        if (char.level < (t.minLevel || 1) || char.level > (t.maxLevel || 999))
            return false;
        return true;
    });
    if (!candidates.length) {
        state.stats.lastActionResult = 'Нет подходящих турниров';
        return;
    }
    ...
    for (const t of candidates) {
        ...
        if (entryFee > 0 && char.money < entryFee)
            continue; // не хватает на взнос
        try {
            const body = {};
            if (t.type === 'official') {
...
// ─── Лечение ───
async function doRest(token, botId, state) {
    const tavern = await apiGet(token, '/tavern');
    const missing = tavern.maxHp - tavern.currentHp;
    if (missing <= 0) {
        state.stats.lastActionResult = 'HP полное';
        return;
    }
    if (tavern.money < missing * 2) {
        state.stats.lastActionResult = 'Нет денег на лечение';
        return;
    }
...
// ─── Цикл ───
async function botLoop(botId, state) {
    ...
    while (!stopFlags.get(botId)) {
        ...
        if (fn) {
            try {
                await fn(token, botId, state);
                state.stats.actions++;
                state.stats.lastAction = chosen;
            }
            catch (e) {
                state.stats.lastActionResult = `Ошибка: ${e.message}`;
            }
...
// ─── Публичные API ───
async function startBots(count, useExisting = true) {
    if (globalRunning && bots.size > 0)
        await stopBots();
    let accounts = [];
    if (useExisting) {
        const stored = await getStoredBots();
        if (stored.length)
            accounts = stored.slice(0, count);
    }
    while (accounts.length < count) {
        try {
            accounts.push(await registerBot());
        }
        catch (e) {
            console.error('[Bots] reg err:', e.message);
            break;
        }
    ...
    for (const acc of accounts) {
        if (acc.username.startsWith('Гость_')) {
            const newName = (0, botNames_1.pickBotName)();
            if (newName) {
                try {
                    await index_1.db.run('UPDATE users SET username = ? WHERE id = ?', [newName, acc.id]);
                    await index_1.db.run('UPDATE bot_accounts SET username = ? WHERE userid = ?', [newName, acc.id]);
    ...
    for (let i = 0; i < accounts.length; i++) {
        ...
        const state = { account: acc, running: true, cooldowns: {}, stats: { actions: 0, lastAction: '', lastActionResult: 'Запущен' } };
        bots.set(acc.id, state);
        setTimeout(() => { if (!stopFlags.get(acc.id))
            botLoop(acc.id, state).catch(e => console.error(`[Bot#${acc.id}] err:`, e.message)); }, i * 3000);
    }
...
async function stopBots() {
    ...
    for (const [id, state] of bots) {
        ...
        if (name && !name.startsWith('Гость_'))
            (0, botNames_1.releaseBotName)(name);
        count++;
    }
    bots.clear();
