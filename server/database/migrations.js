...
function runMigrations(db) {
    ...
    try {
        db.exec('ALTER TABLE users ADD COLUMN statPoints INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN baseS INTEGER DEFAULT 5');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN baseA INTEGER DEFAULT 5');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN baseD INTEGER DEFAULT 5');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN baseM INTEGER DEFAULT 5');
    }
    catch { }
    ...
    try {
        db.exec('ALTER TABLE users ADD COLUMN emailCode TEXT');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN emailCodeExpires INTEGER DEFAULT 0');
    }
    catch { }
    // OAuth провайдеры
    try {
    ...
    // Стоимость предмета
    try {
        db.exec('ALTER TABLE items ADD COLUMN cost INTEGER DEFAULT NULL');
    }
    catch { }
    // Кулдаун PvE атак
    try {
        db.exec('ALTER TABLE users ADD COLUMN lastPveAttackTime INTEGER DEFAULT 0');
    }
    catch { }
    // Банк (сокровищница)
    try {
    ...
    try {
        db.exec('ALTER TABLE users ADD COLUMN lastBankVisit INTEGER DEFAULT 0');
    }
    catch { }
    // Трактир: комната отдыха и напитки
    try {
        db.exec('ALTER TABLE users ADD COLUMN roomType TEXT DEFAULT NULL');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN roomUntil INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN activeDrink TEXT DEFAULT NULL');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN drinkUntil INTEGER DEFAULT 0');
    }
    catch { }
    // ELO рейтинг
    try {
    ...
    try {
        db.exec('ALTER TABLE users ADD COLUMN seasonWins INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN seasonLosses INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN lastEloDecay INTEGER DEFAULT 0');
    }
    catch { }
    // Аукцион
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS auction_lots (
    ...
    try {
        db.exec('CREATE INDEX IF NOT EXISTS idx_auction_lots_seller ON auction_lots(sellerId)');
    }
    catch { }
    try {
        db.exec('CREATE INDEX IF NOT EXISTS idx_auction_lots_ends ON auction_lots(endsAt)');
    }
    catch { }
    // Турнир
    try {
    ...
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS order_members (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    // Бан игроков админом
    try {
        db.exec('ALTER TABLE users ADD COLUMN bannedUntil INTEGER DEFAULT 0');
    }
    catch { }
    // Премиум
    try {
        db.exec('ALTER TABLE users ADD COLUMN premiumUntil INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE job_history ADD COLUMN premiumBonus INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE job_history ADD COLUMN xpGained INTEGER DEFAULT 0');
    }
    catch { }
    ...
    // Гостевые аккаунты
    try {
        db.exec('ALTER TABLE users ADD COLUMN isGuest INTEGER DEFAULT 0');
    }
    catch { }
    // Аватар пользователя
    try {
    ...
    try {
        db.exec('CREATE INDEX IF NOT EXISTS idx_users_is_guest ON users(isGuest)');
    }
    catch { }
    try {
        db.exec('CREATE INDEX IF NOT EXISTS idx_users_active_job ON users(activeJob)');
    }
    catch { }
    // Переименование weapon2 → shield в equipment игроков
    const allUsersEquip = db.prepare('SELECT id, equipment FROM users').all();
    ...
    try {
        db.exec('ALTER TABLE users ADD COLUMN lastPvpTime INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN lastPveRatingTime INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN lastBossKillDate TEXT DEFAULT NULL');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN pveRating INTEGER DEFAULT 0');
    }
    catch { }
    // Таблица сезонов
    try {
    ...
    try {
        db.exec('CREATE INDEX IF NOT EXISTS idx_login_logs_user ON login_logs(userId, createdAt DESC)');
    }
    catch { }
    // Переименование материалов из старых названий в новые (из ideas)
    const materialRenames = {
    ...
    const allUsers = db.prepare('SELECT id, inventory FROM users').all();
    for (const u of allUsers) {
        ...
        if (changed)
            db.prepare('UPDATE users SET inventory = ? WHERE id = ?').run(JSON.stringify(inv), u.id);
    }
    // --- Расширенная статистика игроков ---
    try {
        db.exec('ALTER TABLE users ADD COLUMN pveTotalBattles INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN pveWins INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN tournamentCount INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN tournamentWins INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN totalJobMoney INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN totalPveMoneyWon INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN totalPvpMoneyWon INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN totalPveMoneyLost INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN totalPvpMoneyLost INTEGER DEFAULT 0');
    }
    catch { }
    // --- Скрытый Elo для турнирного посева ---
    try {
        db.exec('ALTER TABLE users ADD COLUMN tournamentElo INTEGER DEFAULT 1000');
    }
    catch { }
    // --- Самоорганизованные турниры ---
    try {
        db.exec("ALTER TABLE tournaments ADD COLUMN type TEXT DEFAULT 'official'");
    }
    catch { }
    try {
        db.exec('ALTER TABLE tournaments ADD COLUMN creatorId INTEGER');
    }
    catch { }
    try {
        db.exec('ALTER TABLE tournaments ADD COLUMN entryFee INTEGER DEFAULT 0');
    }
    catch { }
    try {
    ...
    try {
        db.exec('ALTER TABLE tournaments ADD COLUMN minLevel INTEGER DEFAULT 1');
    }
    catch { }
    try {
        db.exec('ALTER TABLE tournaments ADD COLUMN maxLevel INTEGER DEFAULT 999');
    }
    catch { }
    try {
        db.exec('ALTER TABLE tournaments ADD COLUMN basePool INTEGER DEFAULT 0');
    }
    catch { }
    // --- Банковские счета ---
    try {
        db.exec('ALTER TABLE users ADD COLUMN accountNumber TEXT');
    }
    catch { }
    try {
        db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_accountNumber ON users(accountNumber)');
    }
    catch { }
    // --- Ежедневные квесты ---
    try {
    ...
    try {
        db.exec('ALTER TABLE users ADD COLUMN craftCount INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN auctionTrades INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN totalJobSeconds INTEGER DEFAULT 0');
    }
    catch { }
    // --- Статистика крафта ---
    try {
        db.exec('ALTER TABLE users ADD COLUMN craftCreated INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN craftUpgraded INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE users ADD COLUMN craftBroken INTEGER DEFAULT 0');
    }
    catch { }
    // --- Унификация xpGained → expGained в pve_battles ---
    try {
        db.exec('ALTER TABLE pve_battles RENAME COLUMN xpGained TO expGained');
    }
    catch { }
    ...
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS bank_operations (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    userId INTEGER NOT NULL,
    type TEXT NOT NULL,
    amount INTEGER NOT NULL,
    ...
    const usersWithout = db.prepare('SELECT id FROM users WHERE accountNumber IS NULL').all();
    for (const u of usersWithout) {
        let code;
        do {
            code = genCode();
        } while (db.prepare('SELECT id FROM users WHERE accountNumber = ?').get(code));
        db.prepare('UPDATE users SET accountNumber = ? WHERE id = ?').run(code, u.id);
    }
    // --- История PvE-боёв ---
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS pve_battles (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    userId INTEGER NOT NULL,
    ...
    // --- Гильдии ---
    try {
        db.exec('ALTER TABLE users ADD COLUMN guildId INTEGER DEFAULT NULL REFERENCES guilds(id)');
    }
    catch { }
    try {
    ...
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS guild_invites (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    guildId INTEGER NOT NULL,
    userId INTEGER NOT NULL,
    ...
    try {
        db.exec('ALTER TABLE users ADD COLUMN arenaOpponentId INTEGER DEFAULT NULL');
    }
    catch { }
    // --- Премиум-бонус в истории PvE-боёв ---
    try {
        db.exec('ALTER TABLE pve_battles ADD COLUMN premiumBonus INTEGER DEFAULT 0');
    }
    catch { }
    // --- Фоновое изображение для работ ---
    try {
        db.exec('ALTER TABLE jobs ADD COLUMN background TEXT');
    }
    catch { }
    // --- Изображения и описания для мобов ---
    try {
        db.exec('ALTER TABLE mobs ADD COLUMN background TEXT');
    }
    catch { }
    try {
        db.exec('ALTER TABLE mobs ADD COLUMN description TEXT');
    }
    catch { }
    // --- Таблица действий (action cards) ---
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS actions_config (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    try {
        db.exec('ALTER TABLE guilds ADD COLUMN taxRate INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS guild_treasury_log (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    guildId INTEGER NOT NULL,
    userId INTEGER NOT NULL,
    amount INTEGER NOT NULL,
    ...
    try {
        db.exec('ALTER TABLE guild_treasury_log ADD COLUMN type TEXT NOT NULL DEFAULT \'deposit\'');
    }
    catch { }
    // --- Гильд-войны ---
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS guild_wars (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    try {
        db.exec(`CREATE TABLE IF NOT EXISTS guild_war_attacks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    try {
        db.exec('ALTER TABLE guild_wars ADD COLUMN attackerScore INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE guild_wars ADD COLUMN defenderScore INTEGER DEFAULT 0');
    }
    catch { }
    try {
        db.exec('ALTER TABLE guild_war_attacks ADD COLUMN battleLog TEXT');
    }
    catch { }
    // --- completedAt для турниров ---
    try {
        db.exec('ALTER TABLE tournaments ADD COLUMN completedAt DATETIME');
    }
    catch { }
    ...
    const hasRarityCol = db.prepare("PRAGMA table_info(upgrade_chances)").all().some((c) => c.name === 'rarity_id');
    if (!hasRarityCol) {
        try {
        ...
        PRIMARY KEY (level, rarity_id)
            ...
            const old = db.prepare('SELECT * FROM upgrade_chances').all();
            for (const r of old) {
