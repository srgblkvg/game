...
async function runSeed() {
    // Начальные редкости
    const rarityCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM rarities')).cnt;
    if (rarityCount === 0) {
        const INSERT_RARITY = 'INSERT INTO rarities (id, name, display_name, color) VALUES (?, ?, ?, ?)';
        await index_1.db.run(INSERT_RARITY, [0, 'junk', 'Хлам', '#888888']);
        await index_1.db.run(INSERT_RARITY, [1, 'common', 'Обычный', '#cccccc']);
        await index_1.db.run(INSERT_RARITY, [2, 'uncommon', 'Необычный', '#2ecc71']);
        await index_1.db.run(INSERT_RARITY, [3, 'rare', 'Редкий', '#3498db']);
        await index_1.db.run(INSERT_RARITY, [4, 'epic', 'Эпический', '#9b59b6']);
        await index_1.db.run(INSERT_RARITY, [5, 'legendary', 'Легендарный', '#f1c40f']);
        await index_1.db.run(INSERT_RARITY, [6, 'mythic', 'Мифический', '#e74c3c']);
    }
    // Начальные предметы (189 предметов — по 21 на каждый слот)
    const itemCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM items')).cnt;
    if (itemCount === 0) {
        const INSERT_ITEM = 'INSERT INTO items (name, slot, rarity_id, bonuses, extra, image, cost) VALUES (?, ?, ?, ?, ?, ?, ?)';
        const allItems = [
        ...
        for (const item of allItems) {
            ...
            await index_1.db.run(INSERT_ITEM, [item.name, item.slot, item.rarity_id, JSON.stringify(item.bonuses), JSON.stringify(item.extra), image, item.cost]);
        }
    }
    // Начальные работы (20 работ) — добавляем только если таблица пуста
    const jobCount = (await index_1.db.one('SELECT COUNT(*) as c FROM jobs')).c;
    if (jobCount === 0) {
        const INSERT_JOB = 'INSERT INTO jobs (name, description, duration, rewardMin, rewardMax) VALUES (?, ?, ?, ?, ?)';
        const jobs = [
        ...
        for (const job of jobs) {
            await index_1.db.run(INSERT_JOB, [job.name, job.description, job.duration, job.rewardMin, job.rewardMax]);
        }
    }
    // Начальные названия характеристик
    const statCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM stat_names')).cnt;
    if (statCount === 0) {
        ...
        for (const [name, nameRu] of stats)
            await index_1.db.run(INSERT_STAT, [name, nameRu]);
    }
    // Начальные ресурсы (материалы + камни улучшения)
    const craftItemCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM craft_items')).cnt;
    if (craftItemCount === 0) {
        ...
        for (let i = 0; i < materialNames.length; i++)
            await index_1.db.run(INSERT_CRAFT, [materialNames[i], i, 'craft', null]);
        ...
        for (let i = 0; i < stoneNames.length; i++)
            await index_1.db.run(INSERT_CRAFT, [stoneNames[i], i, 'upgrade', null]);
    }
    // Начальные категории рецептов
    const catCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM craft_recipe_categories')).cnt;
    if (catCount === 0) {
        await index_1.db.run('INSERT INTO craft_recipe_categories (name, sort_order) VALUES (?, ?)', ['Материалы', 1]);
        await index_1.db.run('INSERT INTO craft_recipe_categories (name, sort_order) VALUES (?, ?)', ['Улучшения', 2]);
    }
    // Категория «Предметы» (добавляем если нет)
    try {
        await index_1.db.run('INSERT INTO craft_recipe_categories (name, sort_order) VALUES (?, ?)', ['Предметы', 3]);
    }
    catch { }
    // Начальные шансы улучшения (по редкости и уровню)
    const upgradeChanceCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM upgrade_chances')).cnt;
    // Если данных нет или только одна редкость — перезаполняем все
    if (upgradeChanceCount < 70) {
        await index_1.db.run('DELETE FROM upgrade_chances');
        ...
        const chances = [100, 90, 70, 50, 25, 10, 5, 3, 2, 1];
        const costs = {
        ...
        for (let rarity = 0; rarity <= 6; rarity++) {
            for (let lvl = 0; lvl < 10; lvl++) {
                await index_1.db.run(INSERT_UPGRADE, [lvl + 1, rarity, chances[lvl], costs[rarity][lvl]]);
            }
        }
    }
    // Начальные рецепты крафта (материалы + камни улучшения)
    const recipeCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM craft_recipes')).cnt;
    if (recipeCount === 0) {
        const INSERT_RECIPE = 'INSERT INTO craft_recipes (name, description, money_cost, result_type, result_id, success_chance, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)';
        const INSERT_INGREDIENT = 'INSERT INTO craft_recipe_ingredients (recipe_id, craft_item_id, quantity) VALUES (?, ?, ?)';
        const getCraftItemId = async (name) => (await index_1.db.one('SELECT id FROM craft_items WHERE name = ?', [name])).id;
        const getCatId = async (name) => (await index_1.db.one('SELECT id FROM craft_recipe_categories WHERE name = ?', [name])).id;
        ...
        for (const r of allRecipes) {
            const info = await index_1.db.run(INSERT_RECIPE, [r.name, r.description, r.cost, 'craft_item', await getCraftItemId(r.result), r.chance, r.categoryId]);
            const recipeId = info.lastInsertRowid;
            for (const ing of r.ingredients) {
                await index_1.db.run(INSERT_INGREDIENT, [recipeId, await getCraftItemId(ing.name), ing.qty]);
            }
        }
    }
    // Рецепты случайных предметов (всегда проверяем, добавляем если нет)
    {
        const itemRecipeCount = (await index_1.db.one("SELECT COUNT(*) as cnt FROM craft_recipes WHERE result_type = 'random_item'")).cnt;
        if (itemRecipeCount === 0) {
            ...
            for (const r of itemRecipes) {
                const info = await index_1.db.run(INSERT_RECIPE, [r.name, r.description, r.cost, 'random_item', r.rarity, r.chance, itemCatId]);
                const recipeId = info.lastInsertRowid;
                for (const ing of r.ingredients) {
                    await index_1.db.run(INSERT_INGREDIENT, [recipeId, await getCraftItemId(ing.name), ing.qty]);
                }
    ...
    // Мобы (PvE бестиарий)
    const mobCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM mobs')).cnt;
    if (mobCount === 0) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        const mobs = [
            ...
            // Уровни 100+ (Ад II/III)
            ['Бес-кровопускатель', 110, 1700, 290, 110, 245, 120, 3, 400, 800, 0, 0, 0, 0, 0.05, 0.15, 0.1667, 'Огненные чертоги'],
            ['Одержимый рыцарь', 115, 1900, 310, 105, 270, 125, 3, 400, 800, 0, 0, 0, 0, 0.05, 0.15, 0.1667, 'Огненные чертоги'],
            ['Инкуб-искуситель', 118, 1850, 280, 140, 230, 140, 3, 400, 800, 0, 0, 0, 0, 0.05, 0.15, 0.1667, 'Тронный зал'],
            ['Архидемон', 122, 2200, 340, 120, 280, 135, 3, 400, 800, 0, 0, 0, 0, 0.05, 0.15, 0.1667, 'Тронный зал'],
            ['Василиск адский', 125, 2400, 350, 130, 310, 140, 4, 500, 1000, 0, 0, 0, 0, 0, 0.0667, 0.2667, 'Ледяная бездна'],
            ['Лорд пламени', 130, 2700, 380, 140, 300, 160, 4, 500, 1000, 0, 0, 0, 0, 0, 0.0667, 0.2667, 'Ледяная бездна'],
            ['Костяной дракон', 135, 3000, 390, 120, 350, 155, 4, 500, 1000, 0, 0, 0, 0, 0, 0.0667, 0.2667, 'Ледяная бездна'],
            ['Падший серафим', 140, 3400, 430, 160, 340, 180, 4, 500, 1000, 0, 0, 0, 0, 0, 0.0667, 0.2667, 'Престол падших'],
        ];
        for (const m of mobs)
            await index_1.db.run(INSERT_MOB, m);
    }
    // Стандартные сеты коллекций (7 редкостей)
    const collectionSetCount = (await index_1.db.one('SELECT COUNT(*) as cnt FROM collection_sets')).cnt;
    if (collectionSetCount === 0) {
        ...
        for (let rarity = 0; rarity <= 6; rarity++) {
            const info = await index_1.db.run(INSERT_SET, [raritySetNames[rarity], `Все предметы редкости ${rarity}`, rarity + 1, rarity + 1]);
            const setId = info.lastInsertRowid;
            const rarityItems = await index_1.db.query('SELECT name, slot FROM items WHERE rarity_id = ?', [rarity]);
            for (const item of rarityItems) {
                await index_1.db.run(INSERT_SET_ITEM, [setId, item.name, item.slot]);
            }
        }
    }
}
